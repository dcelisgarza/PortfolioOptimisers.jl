...
struct HierarchicalOptimiser{T1, T2, T3, T4, T5, T6, T7, T8} <:
    ...
    wf::T7
    strict::T8
    function HierarchicalOptimiser(pe::PrE_Pr, clr::HClE_HCl, slv::Option{<:Slv_VecSlv},
        ...
        return new{typeof(pe), typeof(clr), typeof(slv), typeof(wb), typeof(fees),
                   typeof(sets), typeof(wf), typeof(strict)}(pe, clr, slv, wb, fees, sets,
                                                             wf, strict)
    end
end
function HierarchicalOptimiser(; pe::PrE_Pr = EmpiricalPrior(),
                               ...
                               strict::Bool = false)
    return HierarchicalOptimiser(pe, clr, slv, wb, fees, sets, wf, strict)
end
...
function factory(opt::HierarchicalOptimiser, w::AbstractVector)
    return HierarchicalOptimiser(; pe = opt.pe, clr = opt.clr, slv = opt.slv, wb = opt.wb,
...
function opt_view(hco::HierarchicalOptimiser, i)
    ...
    return HierarchicalOptimiser(; pe = pe, clr = hco.clr, slv = hco.slv, wb = wb,
