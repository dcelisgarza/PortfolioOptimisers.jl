...
function set_max_ratio_f_return_constraints!(args...)
    return nothing
end
function set_max_ratio_log_ret_return_constraints!(model::JuMP.Model, ::MaximumRatio)
    sc = model[:sc]
    ohf = model[:ohf]
    risk = model[:risk]
    JuMP.@constraint(model, sr_log_ret_risk, sc * (risk - ohf) <= 0)
end
function set_return_constraints!(model::JuMP.Model, pret::LogarithmicReturn,
    ...
    T = length(X)
    JuMP.@variable(model, t_log_ret[1:T])
    wi = nothing_scalar_array_selector(pret.w, pr.w)
    if isnothing(wi)
        JuMP.@expression(model, ret, Statistics.mean(t_log_ret))
    else
        JuMP.@expression(model, ret, Statistics.mean(t_log_ret, wi))
    end
    add_fees_to_ret!(model, ret)
    add_market_impact_cost!(model, ret)
    set_max_ratio_log_ret_return_constraints!(model, obj)
    JuMP.@expression(model, kret, k .+ X)
    JuMP.@constraint(model, log_ret_ret[i = 1:T],
                     [sc * t_log_ret[i], sc * k, sc * kret[i]] in
