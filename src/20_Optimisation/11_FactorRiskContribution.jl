...
struct FactorRiskContribution{T1, T2, T3, T4, T5, T6, T7, T8, T9} <:
    ...
    function FactorRiskContribution(opt::JuMPOptimiser, re::RegE_Reg, r::RM_VecRM,
        ...
        return new{typeof(opt), typeof(re), typeof(r), typeof(obj), typeof(pl),
                   typeof(sets), typeof(wi), typeof(flag), typeof(fb)}(opt, re, r, obj, pl,
                                                                       sets, wi, flag, fb)
    end
end
...
function opt_view(frc::FactorRiskContribution, i, X::MatNum)
    ...
    return FactorRiskContribution(; opt = opt, re = re, r = r, obj = frc.obj, pl = frc.pl,
...
function _optimise(frc::FactorRiskContribution, rd::ReturnsResult = ReturnsResult();
    ...
    set_tracking_error_constraints!(model, pr, frc.opt.tr, frc, pl, fees, b1; rd = rd)
    ...
    frc_plg = phylogeny_constraints(frc.pl, rd.F)
