struct RandomisedSearchCrossValidation{T1, T2, T3, T4, T5, T6, T7, T8, T9, T10} <:
    ...
    train_score::T9
    kwargs::T10
    function RandomisedSearchCrossValidation(p::MultiSCVValType_VecMultiSCVValType,
...
function make_p_grid(p::PairSCV, n_iter::Integer, rng::Random.AbstractRNG,
                     replace::Bool = false)
    vals = StatsBase.sample(rng, p[2], min(length(p[2]), n_iter); replace = replace)
    return Pair(p[1], vals)
end
function make_p_grid(p::Pair{<:AbstractString, <:Distributions.Distribution},
                     n_iter::Integer, rng::Random.AbstractRNG, args...)
    vals = rand(rng, p[2], n_iter)
    return Pair(p[1], vals)
end
function make_p_grid(ps::AbstractVector{<:Pair{<:AbstractString,
                                               <:Union{<:AbstractVector,
                                                       <:Distributions.Distribution}}},
                     n_iter::Integer, rng::Random.AbstractRNG)
    replace = any(x->isa(x[2], Distributions.Distribution), ps)
    return [make_p_grid(p, n_iter, rng, replace) for p in ps]
end
function make_p_grid(ps::Dict{<:AbstractString,
                              <:Union{<:AbstractVector, <:Distributions.Distribution}},
                     n_iter::Integer, rng::Random.AbstractRNG)
    replace = any(x->isa(x[2], Distributions.Distribution), ps)
    return Dict(make_p_grid(key => val, n_iter, rng, replace) for (key, val) in ps)
end
function make_p_grid(pss::AbstractVector{<:Union{<:Dict{<:AbstractString,
                                                        <:Union{<:AbstractVector,
                                                                <:Distributions.Distribution}},
                                                 <:AbstractVector{<:Pair{<:AbstractString,
                                                                         <:Union{<:AbstractVector,
                                                                                 <:Distributions.Distribution}}}}},
                     n_iter::Integer, rng::Random.AbstractRNG)
    return [make_p_grid(ps, n_iter, rng) for ps in pss]
end
function search_cross_validation(opt::NonFiniteAllocationOptimisationEstimator,
    ...
    return search_cross_validation(opt,
                                   GridSearchCrossValidation(concrete_typed_array(make_p_grid(rscv.p,
                                                                                              rscv.n_iter,
                                                                                              rscv.rng));
                                                             cv = rscv.cv, r = rscv.r,
                                                             score = rscv.score,
