...
# https://github.com/oxfordcontrol/Clarabel.jl/commit/4915b83e0d900d978681d5e8f3a3a5b8e18086f0#diff-5e94c00ebd2a0ec8f46cd68980b667d3abd707db4bcbef791ef124877ce3fa74
function set_risk_constraints!(model::JuMP.Model, i::Any, r::ArbitraryOrderMoment,
                               opt::RiskJuMPOptimisationEstimator, pr::AbstractPriorResult,
                               args...; kwargs...)
    key = Symbol(:arbitrary_moment_risk_, i)
    w = model[:w]
    k = model[:k]
    sc = model[:sc]
    alpha = r.alg.alpha
    ...
    net_X = set_net_portfolio_returns!(model, pr.X)
    T = length(net_X)
    t, u_arbitrary_moment, v_arbitrary_moment = model[Symbol(:t_arbitrary_moment_, i)], model[Symbol(:u_arbitrary_moment_, i)], model[Symbol(:v_arbitrary_moment_, i)] = @variables(model,
                                                                                                                                                                                    begin
                                                                                                                                                                                        ()
                                                                                                                                                                                        [1:T],
                                                                                                                                                                                        (lower_bound = 0)
    ...
    model[Symbol(:carbitrary_moment_mar_, i)] = @constraint(model,
                                                            sc *
                                                            ((net_X + u_arbitrary_moment) .-
                                                             target) >= 0)
    wi = nothing_scalar_array_selector(r.w, pr.w)
    if isnothing(wi)
        iat = inv(alpha * T^ip)
        ...
        u_arbitrary_moment = model[Symbol(:scaled_u_arbitrary_moment_, i)] = @expression(model,
                                                                                         wi .*
                                                                                         u_arbitrary_moment)
    end
    model[Symbol(:carbitrary_moment_pcone_, i)], model[Symbol(:carbitrary_moment_sumt_, i)] = @constraints(model,
                                                                                                           begin
                                                                                                               [i = 1:T],
                                                                                                               [sc *
                                                                                                                v_arbitrary_moment[i],
                                                                                                                sc *
                                                                                                                t,
                                                                                                                sc *
                                                                                                                u_arbitrary_moment[i]] in
                                                                                                               MOI.PowerCone(ip)
                                                                                                               sum(v_arbitrary_moment) ==
                                                                                                               t
                                                                                                           end)
    arbitrary_moment_risk = model[key] = @expression(model, target + iat * t)
    set_risk_bounds_and_expression!(model, opt, arbitrary_moment_risk, r.settings, key)
    return arbitrary_moment_risk
end
