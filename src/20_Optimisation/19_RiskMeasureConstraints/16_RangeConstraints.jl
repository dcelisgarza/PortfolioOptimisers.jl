...
function set_risk_constraints!(model::JuMP.Model, ::Any, r::Range,
    ...
    if haskey(model, Symbol(prefix, :range_risk))
        return model[Symbol(prefix, :range_risk)]
    end
    sc = get_constraint_scale(model)
    wr_risk = set_wr_risk_expression!(model, pr.X; prefix = prefix)
    net_X = get_net_X(model, prefix)
    br_risk = preg!(model, prefix, :br_risk, JuMP.@variable(model))
    range_risk = preg!(model, prefix, :range_risk,
                       JuMP.@expression(model, wr_risk - br_risk))
    preg!(model, prefix, :cbr, JuMP.@constraint(model, sc * (br_risk .+ net_X) <= 0))
    set_risk_bounds_and_expression!(model, opt, range_risk, r.settings,
                                    Symbol(prefix, :range_risk))
    return range_risk
end
