struct AugmentedBlackLittermanPrior{T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,T13,T14,T15} <:
...
function AugmentedBlackLittermanPrior(;
    ...
    a_views::Union{<:LinearConstraintEstimator,<:BlackLittermanViews},
    f_views::Union{<:LinearConstraintEstimator,<:BlackLittermanViews},
    ...
    return AugmentedBlackLittermanPrior(
        a_pe,
        f_pe,
        mp,
        re,
...
function factory(
    ...
    return AugmentedBlackLittermanPrior(;
        a_pe = factory(pe.a_pe, w),
        f_pe = factory(pe.f_pe, w),
        mp = pe.mp,
        re = pe.re,
        ve = factory(pe.ve, w),
        a_views = pe.a_views,
...
function prior(
    ...
    aug_prior_sigma = hcat(
        vcat(a_prior_sigma, f_prior_sigma * transpose(M)),
        vcat(M * f_prior_sigma, f_prior_sigma),
    )
    aug_P = hcat(
        vcat(P, zeros(size(f_P, 1), size(P, 2))),
        vcat(zeros(size(P, 1), size(f_P, 2)), f_P),
    )
    aug_Q = vcat(Q, f_Q)
    aug_omega = hcat(
        vcat(a_omega, zeros(size(f_omega, 1), size(a_omega, 1))),
    ...
    aug_posterior_mu, aug_posterior_sigma = vanilla_posteriors(
        tau,
        pe.rf,
        aug_prior_mu,
        aug_prior_sigma,
