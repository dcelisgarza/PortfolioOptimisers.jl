...
# Details
  ...
  - Any `Distances.SemiMetric` is accepted, including user-defined ones. Every metric yields a similarity, so no combination throws; a metric whose distances are unbounded gives similarities outside ``[-1,\\,1]`` under the default [`ComplementSimilarity`](@ref), which [`plot_clusters`](@ref) silently clips.
