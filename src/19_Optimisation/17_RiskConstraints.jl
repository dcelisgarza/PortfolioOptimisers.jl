...
function set_risk_upper_bound!(
    model::JuMP.Model,
    ::Union{<:MeanRisk,<:NearOptimalCentering,<:RiskBudgetting},
    r_expr::AbstractJuMPScalar,
    ub::Union{<:AbstractVector,<:Frontier},
    key,
)
    bound_key = Symbol(key, :_ub)
    if !haskey(model, :risk_frontier)
        risk_frontier = @expression(
            model,
            risk_frontier,
            Pair{Symbol,Tuple{<:AbstractJuMPScalar,<:Union{<:AbstractVector,<:Frontier}}}[bound_key=>(r_expr, ub)]
        )
    else
...
function set_risk_bounds_and_expression!(
    model::JuMP.Model,
...
function set_variance_risk_bounds_and_expression!(
    model::JuMP.Model,
    opt::Union{<:MeanRisk,<:NearOptimalCentering,<:RiskBudgetting,<:FactorRiskContribution},
...
function set_risk!(
    ...
    model[Symbol(:csd_risk_soc_, i)] =
        @constraint(model, [sc * sd_risk; sc * G * w] in SecondOrderCone())
    return sd_risk, key
end
...
function set_sdp_variance_risk!(
    model::JuMP.Model,
    i::Any,
    r::Variance,
    pr::AbstractPriorResult,
...
function set_variance_risk!(
    ...
    model[Symbol(key_dev, :_soc)] =
        @constraint(model, [sc * dev; sc * G * w] in SecondOrderCone())
...
function rc_variance_constraints!(
    model::JuMP.Model,
    i::Any,
    rc::LinearConstraint,
    ...
    if !isnothing(rc.A_ineq)
        model[Symbol(rc_key, :_ineq)] =
            @constraint(model, sc * (rc.A_ineq * vsw - rc.B_ineq * variance_risk) <= 0)
    end
    if !isnothing(rc.A_eq)
        model[Symbol(rc_key, :_eq)] =
            @constraint(model, sc * (rc.A_eq * vsw - rc.B_eq * variance_risk) == 0)
    end
    return nothing
end
function set_risk!(
...
function set_risk_constraints!(
    ...
    variance_risk, sdp_flag =
        set_risk!(model, i, r, opt, pr, cplg, nplg, args...; kwargs...)
...
function set_risk_constraints!(
    ...
    rc =
        linear_constraints(r.rc, opt.sets; datatype = eltype(pr.X), strict = opt.opt.strict)
    key = Symbol(:variance_risk_, i)
    set_sdp_frc_constraints!(model)
    W = model[:W]
    sigma = isnothing(r.sigma) ? pr.sigma : r.sigma
    sigma_W =
        model[Symbol(:sigma_W_, i)] = @expression(model, transpose(b1) * sigma * b1 * W)
...
function set_ucs_variance_risk!(
    model::JuMP.Model,
    i::Any,
    ...
    x_eucs, ucs_variance_risk =
        model[Symbol(:x_eucs, i)], model[key] = @expressions(model, begin
            G * vec(WpE)
            tr(sigma * WpE) + k * t_eucs
        end)
    model[Symbol(:ge_soc, i)] =
        @constraint(model, [sc * t_eucs; sc * x_eucs] in SecondOrderCone())
    return ucs_variance_risk, key
end
function set_risk_constraints!(
    model::JuMP.Model,
    ...
    r_sigma = r.sigma
    sigma = pr.sigma
    ucs_variance_risk, key =
        set_ucs_variance_risk!(model, i, sigma_ucs(ucs, rd; kwargs...), r_sigma, sigma)
    set_risk_bounds_and_expression!(model, opt, ucs_variance_risk, r.settings, key)
    return nothing
end
function calc_risk_constraint_target(
    ::LowOrderMoment{<:Any,<:Any,Nothing,<:Any},
    w::AbstractVector,
    mu::AbstractVector,
    args...,
)
...
function calc_risk_constraint_target(
    r::LowOrderMoment{<:Any,<:Any,<:AbstractVector,<:Any},
    w::AbstractVector,
    args...,
)
    return dot(w, r.mu)
...
function calc_risk_constraint_target(
    r::LowOrderMoment{<:Any,<:Any,<:Real,<:Any},
    ::Any,
    ::Any,
    k,
)
...
function set_second_moment_risk!(
    model::JuMP.Model,
    ::QuadRiskExpr,
    ::Any,
    factor::Real,
    ...
    return model[key] =
        @expression(model, factor * dot(second_moment, second_moment)), sqrt(factor)
end
function set_second_moment_risk!(
    model::JuMP.Model,
    ::RSOCRiskExpr,
    ...
    model[Symbol(keyc, i)] = @constraint(
        model,
        [
            sc * tsecond_moment;
            0.5;
...
function second_moment_bound_val(
    alg::SecondMomentAlgorithm,
    ub::AbstractVector,
    factor::Real,
)
...
function set_risk_constraints!(
    model::JuMP.Model,
    i::Any,
    r::LowOrderMoment{<:Any,<:Any,<:Any,<:LowOrderDeviation{<:Any,<:SecondLowerMoment}},
    opt::Union{<:MeanRisk,<:NearOptimalCentering,<:RiskBudgetting},
    pr::AbstractPriorResult,
    args...;
    kwargs...,
    ...
    sqrt_second_lower_moment, second_lower_moment =
        model[bound_key], model[Symbol(:second_lower_moment_, i)] = @variables(model, begin
            ()
            [1:T], (lower_bound = 0)
        end)
    wi = nothing_scalar_array_factory(r.w, pr.w)
    second_lower_moment_risk, factor = if isnothing(wi)
        ...
        wi = sqrt.(wi)
        scaled_second_lower_moment =
            model[Symbol(:scaled_second_lower_moment_, i)] =
                @expression(model, dot(wi, second_lower_moment))
        set_second_moment_risk!(
            model,
            r.alg.alg.alg,
            i,
    ...
    model[Symbol(:csqrt_second_central_moment_soc_, i)],
    model[Symbol(:csecond_lower_moment_mar_, i)] = @constraints(
        model,
        begin
            [
                sc * sqrt_second_lower_moment
                sc * second_lower_moment
            ] in SecondOrderCone()
            sc * ((net_X + second_lower_moment) .- target) >= 0
        end
    )
...
function set_risk_constraints!(
    model::JuMP.Model,
    i::Any,
    r::LowOrderMoment{<:Any,<:Any,<:Any,<:LowOrderDeviation{<:Any,<:SecondCentralMoment}},
    opt::Union{<:MeanRisk,<:NearOptimalCentering,<:RiskBudgetting},
    pr::AbstractPriorResult,
    args...;
    kwargs...,
    ...
    second_central_moment =
        model[Symbol(:second_central_moment_, i)] = @expression(model, net_X .- target)
    sc = model[:sc]
    wi = nothing_scalar_array_factory(r.w, pr.w)
    second_central_moment_risk, factor = if isnothing(wi)
        ...
        scaled_second_central_moment =
            model[Symbol(:scaled_second_central_moment_, i)] =
                @expression(model, dot(wi, second_central_moment))
        set_second_moment_risk!(
            model,
            r.alg.alg.alg,
            i,
    ...
    model[Symbol(:csqrt_second_central_moment_soc, i)] = @constraint(
        model,
        [
            sc * sqrt_second_central_moment
            sc * second_central_moment
        ] in SecondOrderCone()
    )
    ub = second_moment_bound_val(r.alg.alg.alg, r.settings.ub, factor)
    set_variance_risk_bounds_and_expression!(
...
function set_risk_constraints!(
    ...
    T = length(net_X)
    var_risk, z_var =
        model[key], model[Symbol(:z_var_, i)] = @variables(model, begin
            ()
            [1:T], (binary = true)
        end)
    alpha = r.alpha
    wi = nothing_scalar_array_factory(r.w, pr.w)
    if isnothing(wi)
        model[Symbol(:csvar_, i)] =
            @constraint(model, sc * (sum(z_var) - alpha * T + s * T) <= 0)
    else
        sw = sum(wi)
        model[Symbol(:csvar_, i)] =
            @constraint(model, sc * (dot(wi, z_var) - alpha * sw + s * sw) <= 0)
    end
    model[Symbol(:cvar_, i)] =
        @constraint(model, sc * ((net_X + b * z_var) .+ var_risk) >= 0)
...
function set_risk_constraints!(
    ...
    net_X = set_net_portfolio_returns!(model, pr.X)
    T = length(net_X)
    var_risk_l, z_var_l, var_risk_h, z_var_h =
        model[Symbol(:var_risk_l_, i)],
        model[Symbol(:z_var_l_, i)],
        model[Symbol(:var_risk_h_, i)],
        model[Symbol(:z_var_h_, i)] = @variables(model, begin
            ()
            [1:T], (binary = true)
            ()
            [1:T], (binary = true)
    ...
    wi = nothing_scalar_array_factory(r.w, pr.w)
    if isnothing(wi)
        model[Symbol(:csvar_l_, i)], model[Symbol(:csvar_h_, i)] = @constraints(model, begin
            sc * (sum(z_var_l) - alpha * T + s * T) <= 0
            sc * (sum(z_var_h) - beta * T + s * T) >= 0
        end)
    else
        sw = sum(wi)
        model[Symbol(:csvar_l_, i)], model[Symbol(:csvar_h_, i)] = @constraints(model, begin
            sc * (dot(wi, z_var_l) - alpha * sw + s * sw) <= 0
            sc * (dot(wi, z_var_h) - beta * sw + s * sw) >= 0
        end)
    end
    model[Symbol(:cvar_, i)] = @constraints(model, begin
        sc * ((net_X + b * z_var_l) .+ var_risk_l) >= 0
        sc * ((net_X + b * z_var_h) .+ var_risk_h) <= 0
    end)
...
function set_risk_constraints!(
    ...
    model[Symbol(:cvar_soc_, i)] =
        @constraint(model, [sc * g_var; sc * G * w] in SecondOrderCone())
    set_risk_bounds_and_expression!(model, opt, var_risk, r.settings, key)
    return nothing
end
function set_risk_constraints!(
    ...
    key = Symbol(:var_range_risk_, i)
    g_var = model[Symbol(:g_var_, i)] = @variable(model)
    var_risk_l, var_risk_h =
        model[Symbol(:var_risk_l_, i)], model[Symbol(:var_risk_h_, i)] =
            @expressions(model, begin
                z_l * g_var
                z_h * g_var
            end)
    var_range_risk = model[key] = @expression(model, var_risk_l - var_risk_h)
    model[Symbol(:cvar_range_soc_, i)] = @constraints(model, begin
        [sc * g_var; sc * G * w] in SecondOrderCone()
    end)
    set_risk_bounds_and_expression!(model, opt, var_range_risk, r.settings, key)
    return nothing
end
function set_risk_constraints!(
    ...
    T = length(net_X)
    var, z_cvar =
        model[Symbol(:z_cvar_, i)], model[Symbol(:var_, i)] = @variables(model, begin
            ()
            [1:T], (lower_bound = 0)
        end)
    wi = nothing_scalar_array_factory(r.w, pr.w)
...
function set_risk_constraints!(
    ...
    net_X = set_net_portfolio_returns!(model, pr.X)
    T = length(net_X)
    var_l, z_cvar_l, var_h, z_cvar_h =
        model[Symbol(:var_l_, i)],
        model[Symbol(:z_cvar_l_, i)],
        model[Symbol(:var_h_, i)],
        model[Symbol(:z_cvar_h_, i)] = @variables(model, begin
            ()
            [1:T], (lower_bound = 0)
            ()
            [1:T], (upper_bound = 0)
    ...
    cvar_risk_l, cvar_risk_h = if isnothing(wi)
        iat = inv(r.alpha * T)
        ibt = inv(r.beta * T)
        model[Symbol(:cvar_risk_l_, i)], model[Symbol(:cvar_risk_h_, i)] =
            @expressions(model, begin
                var_l + sum(z_cvar_l) * iat
                var_h + sum(z_cvar_h) * ibt
            end)
    else
        sw = sum(wi)
        iat = inv(r.alpha * sw)
        ...
        model[Symbol(:cvar_risk_l_, i)], model[Symbol(:cvar_risk_h_, i)] =
            @expressions(model, begin
                var_l + dot(wi, z_cvar_l) * iat
                var_h + dot(wi, z_cvar_h) * ibt
            end)
    end
    cvar_range_risk = model[key] = @expression(model, cvar_risk_l - cvar_risk_h)
    model[Symbol(:ccvar_l_, i)], model[Symbol(:ccvar_h_, i)] = @constraints(model, begin
        sc * ((z_cvar_l + net_X) .+ var_l) >= 0
        sc * ((z_cvar_h + net_X) .+ var_h) <= 0
    end)
...
function set_risk_constraints!(
    ...
    lb, tau, s, tu_drcvar, tv_drcvar, u, v =
        model[Symbol(:lb_, i)],
        model[Symbol(:tau_, i)],
        model[Symbol(:s_, i)],
        model[Symbol(:tu_drcvar_, i)],
        model[Symbol(:tv_drcvar_, i)],
        model[Symbol(:u_, i)],
        model[Symbol(:v_, i)] = @variables(model, begin
            ()
            ()
            [1:T]
            [1:T]
    ...
    model[Symbol(:cu_drcvar_, i)],
    model[Symbol(:cv_drcvar_, i)],
    model[Symbol(:cu_drcvar_infnorm_, i)],
    model[Symbol(:cv_drcvar_infnorm_, i)],
    model[Symbol(:cu_drcvar_lb_, i)],
    model[Symbol(:cv_drcvar_lb_, i)] = @constraints(
        model,
        begin
            sc * (b1 * tau .+ (a1 * net_X + vec(sum(u .* Xap1; dims = 2)) - s)) <= 0
            sc * (b2 * tau .+ (a2 * net_X + vec(sum(v .* Xap1; dims = 2)) - s)) <= 0
            [i = 1:T],
            [
                sc * tu_drcvar[i]
                sc * (-view(u, i, :) - a1 * w)
            ] in MOI.NormInfinityCone(1 + N)
            [i = 1:T],
            [
                sc * tv_drcvar[i]
                sc * (-view(v, i, :) - a2 * w)
...
function set_risk_constraints!(
    ...
    u_h,
    v_h =
        model[Symbol(:lb_l_, i)],
        model[Symbol(:tau_l_, i)],
        model[Symbol(:s_l_, i)],
        model[Symbol(:tu_drcvar_l_, i)],
        model[Symbol(:tv_drcvar_l_, i)],
        model[Symbol(:u_l_, i)],
        model[Symbol(:v_l_, i)],
        model[Symbol(:lb_h_, i)],
        model[Symbol(:tau_h_, i)],
        model[Symbol(:s_h_, i)],
        model[Symbol(:tu_drcvar_h_, i)],
        model[Symbol(:tv_drcvar_h_, i)],
        model[Symbol(:u_h_, i)],
        model[Symbol(:v_h_, i)] = @variables(model, begin
            ()
            ()
            [1:T]
            [1:T]
            ...
            [1:T]
            [1:T]
            [1:T, 1:N], (upper_bound = 0)
            [1:T, 1:N], (upper_bound = 0)
        end)
    model[Symbol(:cu_drcvar_l_, i)],
    model[Symbol(:cv_drcvar_l_, i)],
    model[Symbol(:cu_drcvar_infnorm_l_, i)],
    model[Symbol(:cv_drcvar_infnorm_l_, i)],
    model[Symbol(:cu_drcvar_lb_l_, i)],
    model[Symbol(:cv_drcvar_lb_l_, i)],
    model[Symbol(:cu_drcvar_h_, i)],
    model[Symbol(:cv_drcvar_h_, i)],
    model[Symbol(:cu_drcvar_infnorm_h_, i)],
    model[Symbol(:cv_drcvar_infnorm_h_, i)],
    model[Symbol(:cu_drcvar_lb_h_, i)],
    model[Symbol(:cv_drcvar_lb_h_, i)] = @constraints(
        model,
        begin
            sc * (b1_l * tau_l .+ (a1_l * net_X + vec(sum(u_l .* Xap1; dims = 2)) - s_l)) <= 0
            sc * (b2_l * tau_l .+ (a2_l * net_X + vec(sum(v_l .* Xap1; dims = 2)) - s_l)) <=
            0
            [i = 1:T],
            [
                sc * tu_drcvar_l[i]
                sc * (-view(u_l, i, :) - a1_l * w)
            ] in MOI.NormInfinityCone(1 + N)
            [i = 1:T],
            [
                sc * tv_drcvar_l[i]
                sc * (-view(v_l, i, :) - a2_l * w)
            ] in MOI.NormInfinityCone(1 + N)
            sc * (tu_drcvar_l .- lb_l) <= 0
            sc * (tv_drcvar_l .- lb_l) <= 0
            sc * (b1_h * tau_h .+ (a1_h * net_X + vec(sum(u_h .* Xap1; dims = 2)) - s_h)) >=
            0
            sc * (b2_h * tau_h .+ (a2_h * net_X + vec(sum(v_h .* Xap1; dims = 2)) - s_h)) >=
            0
            [i = 1:T],
            [
                -sc * tu_drcvar_h[i]
                sc * (view(u_h, i, :) + a1_h * w)
            ] in MOI.NormInfinityCone(1 + N)
            [i = 1:T],
            [
                -sc * tv_drcvar_h[i]
                sc * (view(v_h, i, :) + a2_h * w)
            ] in MOI.NormInfinityCone(1 + N)
            sc * (tu_drcvar_h .- lb_h) >= 0
            sc * (tv_drcvar_h .- lb_h) >= 0
        end
    )
    wi = nothing_scalar_array_factory(r.w, pr.w)
    drcvar_risk_l, drcvar_risk_h =
        model[Symbol(:drcvar_risk_l_, i)], model[Symbol(:drcvar_risk_h_, i)] =
            if isnothing(wi)
                @expressions(model, begin
...
function set_risk_constraints!(
    ...
    net_X = set_net_portfolio_returns!(model, pr.X)
    T = length(net_X)
    t_evar, z_evar, u_evar =
        model[Symbol(:t_evar_, i)], model[Symbol(:z_evar_, i)], model[Symbol(:u_evar_, i)] =
            @variables(model, begin
                ()
                (), (lower_bound = 0)
                [1:T]
    ...
    model[Symbol(:cevar_exp_cone_, i)] = @constraint(
        model,
        [i = 1:T],
        [sc * (-net_X[i] - t_evar), sc * z_evar, sc * u_evar[i]] in MOI.ExponentialCone()
    )
    evar_risk =
        model[Symbol(:evar_risk_, i)] = @expression(model, t_evar - z_evar * log(at))
...
function set_risk_constraints!(
    ...
    net_X = set_net_portfolio_returns!(model, pr.X)
    T = length(net_X)
    t_evar_l, z_evar_l, u_evar_l, t_evar_h, z_evar_h, u_evar_h =
        model[Symbol(:t_evar_l_, i)],
        model[Symbol(:z_evar_l_, i)],
        model[Symbol(:u_evar_l_, i)],
        model[Symbol(:t_evar_h_, i)],
        model[Symbol(:z_evar_h_, i)],
        model[Symbol(:u_evar_h_, i)] = @variables(model, begin
            ()
            (), (lower_bound = 0)
            [1:T]
            ()
    ...
    at, bt = if isnothing(wi)
        model[Symbol(:cevar_l_, i)], model[Symbol(:cevar_h_, i)] =
            @constraints(model, begin
                sc * (sum(u_evar_l) - z_evar_l) <= 0
                sc * (sum(u_evar_h) - z_evar_h) >= 0
            end)
        r.alpha * T, r.beta * T
    else
        sw = sum(wi)
        model[Symbol(:cevar_l_, i)], model[Symbol(:cevar_h_, i)] =
            @constraints(model, begin
                sc * (dot(wi, u_evar_l) - z_evar_l) <= 0
                sc * (dot(wi, u_evar_h) - z_evar_h) >= 0
            end)
        r.alpha * sw, r.beta * sw
    end
    model[Symbol(:cevar_exp_cone_l_, i)], model[Symbol(:cevar_exp_cone_h_, i)] =
        @constraints(
            model,
            begin
                [i = 1:T],
                [sc * (-net_X[i] - t_evar_l), sc * z_evar_l, sc * u_evar_l[i]] in
                MOI.ExponentialCone()
                [i = 1:T],
                [sc * (net_X[i] + t_evar_h), -sc * z_evar_h, -sc * u_evar_h[i]] in
                MOI.ExponentialCone()
            end
        )
    evar_risk_l, evar_risk_h =
        model[Symbol(:evar_risk_l_, i)], model[Symbol(:evar_risk_h_, i)] =
            @expressions(model, begin
                t_evar_l - z_evar_l * log(at)
                t_evar_h - z_evar_h * log(bt)
            end)
...
function set_risk_constraints!(
    ...
    alpha = r.alpha
    kappa = r.kappa
    t_rlvar, z_rlvar, omega_rlvar, psi_rlvar, theta_rlvar, epsilon_rlvar =
        model[Symbol(:t_rlvar_, i)],
        model[Symbol(:z_rlvar_, i)],
        model[Symbol(:omega_rlvar_, i)],
        model[Symbol(:psi_rlvar_, i)],
        model[Symbol(:theta_rlvar_, i)],
        model[Symbol(:epsilon_rlvar_, i)] = @variables(model, begin
            ()
            (), (lower_bound = 0)
            [1:T]
            [1:T]
    ...
    iopk = inv(opk)
    iomk = inv(omk)
    model[Symbol(:crlvar_pcone_a_, i)],
    model[Symbol(:crlvar_pcone_b_, i)],
    model[Symbol(:crlvar_, i)] = @constraints(
        model,
        begin
            [i = 1:T],
            [
                sc * z_rlvar * opk * ik2,
                sc * psi_rlvar[i] * opk * ik,
                sc * epsilon_rlvar[i],
            ] in MOI.PowerCone(iopk)
            [i = 1:T],
            [sc * omega_rlvar[i] * iomk, sc * theta_rlvar[i] * ik, -sc * z_rlvar * ik2] in MOI.PowerCone(omk)
            sc * ((epsilon_rlvar + omega_rlvar - net_X) .- t_rlvar) <= 0
        end
    )
...
function set_risk_constraints!(
    ...
    theta_rlvar_h,
    epsilon_rlvar_h =
        model[Symbol(:t_rlvar_l_, i)],
        model[Symbol(:z_rlvar_l_, i)],
        model[Symbol(:omega_rlvar_l_, i)],
        model[Symbol(:psi_rlvar_l_, i)],
        model[Symbol(:theta_rlvar_l_, i)],
        model[Symbol(:epsilon_rlvar_l_, i)],
        model[Symbol(:t_rlvar_h_, i)],
        model[Symbol(:z_rlvar_h_, i)],
        model[Symbol(:omega_rlvar_h_, i)],
        model[Symbol(:psi_rlvar_h_, i)],
        model[Symbol(:theta_rlvar_h_, i)],
        model[Symbol(:epsilon_rlvar_h_, i)] = @variables(model, begin
            ()
            (), (lower_bound = 0)
            [1:T]
            [1:T]
    ...
    ik2_b = inv(2 * kappa_b)
    wi = nothing_scalar_array_factory(r.w, pr.w)
    rlvar_risk_l, rlvar_risk_h =
        model[Symbol(:rlvar_risk_l_, i)], model[Symbol(:rlvar_risk_h_, i)] =
            if isnothing(wi)
                iat = inv(alpha * T)
                ibt = inv(beta * T)
    ...
    iomk_b = inv(omk_b)
    rlvar_range_risk =
        model[Symbol(:rlvar_range_risk_, i)] =
            @expression(model, rlvar_risk_l - rlvar_risk_h)
    model[Symbol(:crlvar_l_pcone_a_, i)],
    model[Symbol(:crlvar_l_pcone_b_, i)],
    model[Symbol(:crlvar_l_, i)],
    model[Symbol(:crlvar_h_pcone_a_, i)],
    model[Symbol(:crlvar_h_pcone_b_, i)],
    model[Symbol(:crlvar_h_, i)] = @constraints(
        model,
        begin
            [i = 1:T],
            [
                sc * z_rlvar_l * opk_a * ik2_a,
                sc * psi_rlvar_l[i] * opk_a * ik_a,
                sc * epsilon_rlvar_l[i],
            ] in MOI.PowerCone(iopk_a)
            [i = 1:T],
            [
                sc * omega_rlvar_l[i] * iomk_a,
                sc * theta_rlvar_l[i] * ik_a,
                sc * -z_rlvar_l * ik2_a,
            ] in MOI.PowerCone(omk_a)
            sc * ((epsilon_rlvar_l + omega_rlvar_l - net_X) .- t_rlvar_l) <= 0
            [i = 1:T],
            [
                sc * -z_rlvar_h * opk_b * ik2_b,
                sc * -psi_rlvar_h[i] * opk_b * ik_b,
                sc * -epsilon_rlvar_h[i],
            ] in MOI.PowerCone(iopk_b)
            [i = 1:T],
            [
                sc * -omega_rlvar_h[i] * iomk_b,
                sc * -theta_rlvar_h[i] * ik_b,
                sc * z_rlvar_h * ik2_b,
            ] in MOI.PowerCone(omk_b)
            sc * ((epsilon_rlvar_h + omega_rlvar_h - net_X) .- t_rlvar_h) >= 0
        end
    )
...
function set_risk_constraints!(
    ...
    iat = inv(r.alpha * T)
    dar, z_cdar =
        model[Symbol(:dar_, i)], model[Symbol(:z_cdar_, i)] = @variables(model, begin
            ()
            [1:T], (lower_bound = 0)
        end)
    cdar_risk = model[key] = @expression(model, dar + sum(z_cdar) * iat)
    model[Symbol(:ccdar_, i)] =
        @constraint(model, sc * ((z_cdar - view(dd, 2:(T+1))) .+ dar) >= 0)
...
function set_risk_constraints!(
    ...
    T = length(dd) - 1
    at = r.alpha * T
    t_edar, z_edar, u_edar =
        model[Symbol(:t_edar_, i)], model[Symbol(:z_edar_, i)], model[Symbol(:u_edar_, i)] =
            @variables(model, begin
                ()
                (), (lower_bound = 0)
                [1:T]
    ...
    model[Symbol(:cedar_, i)], model[Symbol(:cedar_exp_cone_, i)] = @constraints(
        model,
        begin
            sc * (sum(u_edar) - z_edar) <= 0
            [i = 1:T],
            [sc * (dd[i+1] - t_edar), sc * z_edar, sc * u_edar[i]] in MOI.ExponentialCone()
        end
    )
...
function set_risk_constraints!(
    ...
    iopk = inv(opk)
    iomk = inv(omk)
    t_rldar, z_rldar, omega_rldar, psi_rldar, theta_rldar, epsilon_rldar =
        model[Symbol(:t_rldar_, i)],
        model[Symbol(:z_rldar_, i)],
        model[Symbol(:omega_rldar_, i)],
        model[Symbol(:psi_rldar_, i)],
        model[Symbol(:theta_rldar_, i)],
        model[Symbol(:epsilon_rldar_, i)] = @variables(model, begin
            ()
            (), (lower_bound = 0)
            [1:T]
            [1:T]
    ...
    model[Symbol(:crldar_pcone_a_, i)],
    model[Symbol(:crldar_pcone_b_, i)],
    model[Symbol(:crldar_, i)] = @constraints(
        model,
        begin
            [i = 1:T],
            [
                sc * z_rldar * opk * ik2,
                sc * psi_rldar[i] * opk * ik,
                sc * epsilon_rldar[i],
            ] in MOI.PowerCone(iopk)
            [i = 1:T],
            [sc * omega_rldar[i] * iomk, sc * theta_rldar[i] * ik, -sc * z_rldar * ik2] in MOI.PowerCone(omk)
            sc * ((epsilon_rldar + omega_rldar + view(dd, 2:(T+1))) .- t_rldar) <= 0
        end
    )
...
function set_risk_constraints!(
    model::JuMP.Model,
    i::Any,
    r::SquareRootKurtosis{<:Any,<:Any,<:Any,<:Any,<:Integer,<:Any},
    opt::Union{<:MeanRisk,<:NearOptimalCentering,<:RiskBudgetting},
    pr::HighOrderPrior,
    args...;
    kwargs...,
    ...
    f = clamp(r.N, 1, N)
    Nf = f * N
    sqrt_kurtosis_risk, x_kurt =
        model[key], model[Symbol(:x_kurt_, i)] = @variables(model, begin
            ()
            [1:Nf]
        end)
    ...
    model[Symbol(:capprox_kurt_soc_, i)], model[Symbol(:capprox_kurt_, i)] =
        @constraints(model, begin
            [
                sc * sqrt_kurtosis_risk
                sc * x_kurt
            ] in SecondOrderCone()
            [i = 1:Nf], sc * (x_kurt[i] - tr(Bi[i] * W)) == 0
        end)
...
function set_risk_constraints!(
    model::JuMP.Model,
    i::Any,
    r::SquareRootKurtosis{<:Any,<:Any,<:Any,<:Any,Nothing,<:Any},
    opt::Union{<:MeanRisk,<:NearOptimalCentering,<:RiskBudgetting},
    pr::HighOrderPrior,
    args...;
    kwargs...,
    ...
    model[Symbol(:ckurt_soc_, i)] = @constraint(
        model,
        [
            sc * sqrt_kurtosis_risk;
...
function set_risk_constraints!(
    ...
    throw(
        ArgumentError("SquareRootKurtosis requires a HighOrderPrior, not a $(typeof(pr))."),
    )
end
...
function set_risk_constraints!(
    ...
    owa_a, owa_b =
        model[Symbol(:owa_a_, i)], model[Symbol(:owa_b_, i)] = @variables(model, begin
            [1:T]
            [1:T]
        end)
    owa_risk = model[key] = @expression(model, sum(owa_a + owa_b))
    owa_w = isnothing(r.w) ? owa_gmd(T) : r.w
    model[Symbol(:cowa_, i)] = @constraint(
        model,
        sc * (owa * transpose(owa_w) - ovec * transpose(owa_a) - owa_b * transpose(ovec)) in
        Nonpositives()
    )
...
function set_risk_constraints!(
    ...
    ovec = range(1; stop = 1, length = T)
    owa_a, owa_b =
        model[Symbol(:owa_range_a_, i)], model[Symbol(:owa_range_b_, i)] =
            @variables(model, begin
                [1:T]
                [1:T]
            end)
    ...
    owa_w = owa_w1 - owa_w2
    model[Symbol(:cowa_range_, i)] = @constraint(
        model,
        sc * (owa * transpose(owa_w) - ovec * transpose(owa_a) - owa_b * transpose(ovec)) in
        Nonpositives()
    )
...
function set_risk_constraints!(
    ...
    owa_p = r.alg.p
    M = length(owa_p)
    owa_t, owa_nu, owa_eta, owa_epsilon, owa_psi, owa_z, owa_y =
        model[Symbol(:owa_t_, i)],
        model[Symbol(:owa_nu_, i)],
        model[Symbol(:owa_eta_, i)],
        model[Symbol(:owa_epsilon_, i)],
        model[Symbol(:owa_psi_, i)],
        model[Symbol(:owa_z_, i)],
        model[Symbol(:owa_y_, i)] = @variables(model, begin
            ()
            [1:T], (lower_bound = 0)
            [1:T], (lower_bound = 0)
            [1:T, 1:M]
    ...
    aowa_risk, neg_owa_z_owa_p, owa_p_o_owa_pm1 =
        model[key],
        model[Symbol(:neg_owa_z_owa_p_, i)],
        model[Symbol(:owa_p_o_owa_pm1_, i)] = @expressions(
            model,
            begin
                owa_s * owa_t - owa_l * sum(owa_nu) + owa_h * sum(owa_eta) + dot(owa_d, owa_y)
                -sc * owa_z .* owa_p
                sc * owa_p ./ (owa_p .- one(eltype(owa_p)))
            end
        )
    model[Symbol(:ca1_owa_, i)],
    model[Symbol(:ca2_owa_, i)],
    model[Symbol(:ca_owa_pcone_, i)] = @constraints(
        model,
        begin
            sc * ((net_X - owa_nu + owa_eta - vec(sum(owa_epsilon; dims = 2))) .+ owa_t) == 0
            sc * (owa_z + owa_y - vec(sum(owa_psi; dims = 1))) == 0
            [i = 1:M, j = 1:T],
            [
                neg_owa_z_owa_p[i],
                owa_psi[j, i] * owa_p_o_owa_pm1[i],
                sc * owa_epsilon[j, i],
            ] in MOI.PowerCone(inv(owa_p[i]))
        end
    )
...
function set_risk_constraints!(
    ...
    owa_h_z,
    owa_h_y =
        model[Symbol(:owa_l_t_, i)],
        model[Symbol(:owa_l_nu_, i)],
        model[Symbol(:owa_l_eta_, i)],
        model[Symbol(:owa_l_epsilon_, i)],
        model[Symbol(:owa_l_psi_, i)],
        model[Symbol(:owa_l_z_, i)],
        model[Symbol(:owa_l_y_, i)],
        model[Symbol(:owa_h_t_, i)],
        model[Symbol(:owa_h_nu_, i)],
        model[Symbol(:owa_h_eta_, i)],
        model[Symbol(:owa_h_epsilon_, i)],
        model[Symbol(:owa_h_psi_, i)],
        model[Symbol(:owa_h_z_, i)],
        model[Symbol(:owa_h_y_, i)] = @variables(model, begin
            ()
            [1:T], (lower_bound = 0)
            [1:T], (lower_bound = 0)
            [1:T, 1:M]
    ...
    owa_l_risk, neg_owa_l_z_owa_p, owa_h_risk, neg_owa_h_z_owa_p, owa_p_o_owa_pm1 =
        model[Symbol(:owa_l_risk_, i)],
        model[Symbol(:neg_owa_l_z_owa_p_, i)],
        model[Symbol(:owa_h_risk_, i)],
        model[Symbol(:neg_owa_h_z_owa_p_, i)],
        model[Symbol(:owa_p_o_owa_pm1_, i)] = @expressions(
            model,
            begin
                owa_l_s * owa_l_t - owa_l_l * sum(owa_l_nu) +
                owa_l_h * sum(owa_l_eta) +
                dot(owa_l_d, owa_l_y)
                -sc * owa_l_z .* owa_p
                owa_h_s * owa_h_t - owa_h_l * sum(owa_h_nu) +
                owa_h_h * sum(owa_h_eta) +
                dot(owa_h_d, owa_h_y)
                -sc * owa_h_z .* owa_p
                sc * owa_p ./ (owa_p .- one(eltype(owa_p)))
            end
        )
    model[Symbol(:ca1_owa_l_, i)],
    model[Symbol(:ca2_owa_l_, i)],
    model[Symbol(:ca_owa_pcone_l_, i)],
    model[Symbol(:ca1_owa_h_, i)],
    model[Symbol(:ca2_owa_h_, i)],
    model[Symbol(:ca_owa_pcone_h_, i)] = @constraints(
        model,
        begin
            sc * (
                (net_X - owa_l_nu + owa_l_eta - vec(sum(owa_l_epsilon; dims = 2))) .+
                owa_l_t
            ) == 0
            sc * (owa_l_z + owa_l_y - vec(sum(owa_l_psi; dims = 1))) == 0
            [i = 1:M, j = 1:T],
            [
                neg_owa_l_z_owa_p[i],
                sc * owa_l_psi[j, i] * owa_p_o_owa_pm1[i],
                sc * owa_l_epsilon[j, i],
            ] in MOI.PowerCone(inv(owa_p[i]))
            sc * (
                (-net_X - owa_h_nu + owa_h_eta - vec(sum(owa_h_epsilon; dims = 2))) .+
                owa_h_t
            ) == 0
            sc * (owa_h_z + owa_h_y - vec(sum(owa_h_psi; dims = 1))) == 0
            [i = 1:M, j = 1:T],
            [
                neg_owa_h_z_owa_p[i],
                sc * owa_h_psi[j, i] * owa_p_o_owa_pm1[i],
                sc * owa_h_epsilon[j, i],
            ] in MOI.PowerCone(inv(owa_p[i]))
        end
    )
...
function set_brownian_distance_variance_constraints!(
    model::JuMP.Model,
...
function set_brownian_distance_risk_constraint!(
    model::JuMP.Model,
    ::QuadRiskExpr,
    Dt::AbstractMatrix,
    iT2::Real,
...
function set_risk_constraints!(
    model::JuMP.Model,
    i::Any,
    r::NegativeSkewness{<:Any,<:Any,<:Any,<:Any,<:SqrtRiskExpr},
    opt::Union{<:MeanRisk,<:NearOptimalCentering,<:RiskBudgetting},
    pr::HighOrderPrior,
    args...;
    kwargs...,
    ...
    model[Symbol(:cnskew_soc_, i)] =
        @constraint(model, [sc * nskew_risk; sc * G * w] in SecondOrderCone())
...
function set_risk_constraints!(
    model::JuMP.Model,
    i::Any,
    r::NegativeSkewness{<:Any,<:Any,<:Any,<:Any,<:QuadRiskExpr},
    opt::Union{<:MeanRisk,<:NearOptimalCentering,<:RiskBudgetting},
    pr::HighOrderPrior,
    args...;
    kwargs...,
    ...
    model[Symbol(:cqnskew_soc_, i)] =
        @constraint(model, [sc * t_qnskew_risk; sc * G * w] in SecondOrderCone())
...
function set_risk_constraints!(
    ...
    model[Symbol(:ctracking_r_noc_, i)] = @constraint(
        model,
        [
            sc * t_tracking_risk;
            sc * tracking_r
        ] in MOI.NormOneCone(1 + T)
    )
