...
struct NestedClustering{T1,T2,T3,T4,T5,T6,T7,T8,T9} <: ClusteringOptimisationEstimator
    pe::T1
    cle::T2
    wb::T3
    sets::T4
...
function assert_rc_variance(
    opt::Union{<:MeanRisk,<:FactorRiskContribution,<:NearOptimalCentering,<:RiskBudgetting},
)
    if isa(opt.r, Variance)
        @argcheck(
            !isa(opt.r.rc, LinearConstraint),
    ...
    elseif isa(opt.r, AbstractVector) && any(x -> isa(x, Variance), opt.r)
        ...
        @argcheck(
            !any(x -> isa(x.rc, LinearConstraint), view(opt.r, idx)),
            "`rc` cannot be a `LinearConstraint` because there is no way to only consider items from a specific group and because this would break factor risk contribution"
        )
    end
    return nothing
...
function assert_internal_optimiser(
    opt::AbstractVector{<:Union{<:OptimisationEstimator,<:OptimisationResult}},
)
...
function assert_external_optimiser(
    opt::AbstractVector{<:Union{<:OptimisationEstimator,<:OptimisationResult}},
)
...
function nested_clustering_finaliser(
