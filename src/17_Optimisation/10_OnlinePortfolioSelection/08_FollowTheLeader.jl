...
The paper takes ``\\ell = \\lfloor p_\\ell\\, n \\rfloor`` neighbours, a fraction of the candidates that grows with the history, so `neighbours` is either an `Integer` count or a fraction in `(0, 1)` of the candidate rows; a fraction that rounds to no neighbour is the empty selection. Ties at the boundary are broken by the order of the rows.
