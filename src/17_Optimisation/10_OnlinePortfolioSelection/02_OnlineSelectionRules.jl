...
The rule over rules: every expert takes its own Online Update, and a weighting that is itself an Online Selection Rule moves the weight vector over the experts on their period returns; the answer is the weighted mix of the experts' next allocations (ADR 0156).
