...
function risk_measure_nothing_scalar_array_view(
    risk_variable::Union{<:Real,<:AbstractArray},
    ::Any,
    i::AbstractVector,
)
...
function risk_measure_nothing_scalar_array_view(
    ::Nothing,
    prior_variable::AbstractArray,
    i::AbstractVector,
)
    return nothing_scalar_array_view(prior_variable, i)
end
function risk_measure_nothing_scalar_array_view(
    risk_variable::Union{Nothing,<:Real},
    ::AbstractVector,
)
    return risk_variable
end
...
function solver_factory(::Nothing, ::Nothing)
    throw(
        ArgumentError(
            "Both risk_solver and prior_solver are nothing, cannot solve JuMP model.",
        ),
    )
end
