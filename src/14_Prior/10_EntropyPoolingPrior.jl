...
# Constructor
    ...
    JuMPEntropyPooling(; slv::Union{<:Solver, <:SlvVec}, sc1::Number = 1,
...
struct JuMPEntropyPooling{T1, T2, T3, T4, T5} <: AbstractEntropyPoolingOptimiser
    ...
    so::T4
    alg::T5
    function JuMPEntropyPooling(slv::Union{<:Solver, <:SlvVec}, sc1::Number, sc2::Number,
...
function JuMPEntropyPooling(; slv::Union{<:Solver, <:SlvVec}, sc1::Number = 1,
                            sc2::Number = 1e5, so::Number = 1,
                            alg::AbstractEntropyPoolingOptAlgorithm = ExpEntropyPooling())
...
"""
    fix_mu!(epc::AbstractDict, fixed::NumVec, to_fix::NumVec,
...
# Related
  ...
  - [`EntropyPoolingPrior`](@ref)
"""
function fix_mu!(epc::AbstractDict, fixed::NumVec, to_fix::NumVec, pr::AbstractPriorResult)
    fix = to_fix .& .!fixed
    if any(fix)
...
"""
    fix_sigma!(epc::AbstractDict, fixed::NumVec, to_fix::NumVec,
               pr::AbstractPriorResult)
...
# Related
  ...
  - [`EntropyPoolingPrior`](@ref)
"""
function fix_sigma!(epc::AbstractDict, fixed::NumVec, to_fix::NumVec,
                    pr::AbstractPriorResult)
    sigma = diag(pr.sigma)
    fix = to_fix .& .!fixed
    if any(fix)
