struct FactorPrior{T1 <: AbstractMatrix, T2 <: AbstractVector, T3 <: AbstractMatrix,
...
function FactorPrior(; X::AbstractMatrix, mu::AbstractVector, sigma::AbstractMatrix,
                     chol::AbstractMatrix, f_mu::AbstractVector, f_sigma::AbstractMatrix,
    ...
    return FactorPrior{typeof(X), typeof(mu), typeof(sigma), typeof(chol), typeof(f_mu),
                       typeof(f_sigma), typeof(loadings)}(X, mu, sigma, chol, f_mu, f_sigma,
                                                          loadings)
end
struct FactorPriorEstimator{T1 <: PriorEstimator, T2 <: MatrixProcessing,
                            T3 <: RegressionMethod,
...
function FactorPriorEstimator(; pe::PriorEstimator = EmpiricalPriorEstimator(),
...
export FactorPrior, FactorPriorEstimator
