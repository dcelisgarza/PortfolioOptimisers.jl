...
function Gerber2Covariance(; ve::StatsBase.CovarianceEstimator = SimpleVariance(),
    ...
    return Gerber2Covariance{typeof(ve), typeof(fix_non_pos_def), typeof(threshold)}(ve,
                                                                                     fix_non_pos_def,
                                                                                     threshold)
end
...
function StatsBase.cor(ce::Gerber2Covariance, X::AbstractMatrix; dims::Int = 1)
...
function StatsBase.cov(ce::Gerber2Covariance, X::AbstractMatrix; dims::Int = 1)
    @smart_assert(dims ∈ (1, 2))
    if dims == 2
        X = transpose(X)
    end
