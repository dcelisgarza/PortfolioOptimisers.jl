...
Return the indices of the columns of `X` that hold no `missing` and no `NaN` entry, or of the rows when `dims = 2`.

The test reads those two sentinels and nothing else. An infinite entry is not `NaN`, so a column that holds `Inf` counts as complete.

This is not the rule of [`CompleteAssetSelector`](@ref). That selector keeps the Coverage Universe of the training window, which also drops a column with an infinite entry and a column that the [`AssetPanel`](@ref) marks inactive at some row. To drop observations from a price window, use [`MissingDataFilter`](@ref).
...
# Returns
  ...
  - `res::VecInt`: The indices of the complete columns, or of the complete rows when `dims = 2`, in ascending order.
