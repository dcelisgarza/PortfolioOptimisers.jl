...
for r ∈ traverse_subtypes(RiskMeasure)
    eval(quote
             function no_bounds_no_risk_expr_risk_meausre(r::$(r))
                 pnames = setdiff(propertynames(r), (:settings,))
                 settings = r.settings
...
function no_bounds_no_risk_expr_risk_meausre(r::AbstractVector{<:AbstractBaseRiskMeasure})
    return no_bounds_no_risk_expr_risk_meausre.(r)
end