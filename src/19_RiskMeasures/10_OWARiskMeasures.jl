...
struct ApproxOrderedWeightsArray{T1} <: OrderedWeightsArrayFormulation
    p::T1
    function ApproxOrderedWeightsArray(p::NumVec)
        @argcheck(!isempty(p))
        @argcheck(all(x -> x > one(x), p))
        return new{typeof(p)}(p)
    end
end
function ApproxOrderedWeightsArray(; p::NumVec = Float64[2, 3, 4, 10, 50])
...
function (r::OrderedWeightsArray)(x::NumVec)
...
function (r::OrderedWeightsArrayRange)(x::NumVec)
