struct AverageDrawdown{T1, T2} <: RiskMeasure
    settings::T1
    w::T2
    function AverageDrawdown(settings::RiskMeasureSettings, w::WeightsType)
        if !isnothing(w)
            @argcheck(!isempty(w))
        end
...
function (::AverageDrawdown{<:Any, Nothing})(x::NumVec)
    pushfirst!(x, 1)
    cs = cumsum(x)
    val = zero(eltype(x))
...
function (r::AverageDrawdown{<:Any, <:AbstractWeights})(x::NumVec)
...
function (r::RelativeAverageDrawdown{<:Any, Nothing})(x::NumVec)
...
function (r::RelativeAverageDrawdown{<:Any, <:AbstractWeights})(x::NumVec)
    @argcheck(length(r.w) == length(x))
    x .= pushfirst!(x, 0) .+ one(eltype(x))
    cs = cumprod(x)
    val = zero(eltype(x))
