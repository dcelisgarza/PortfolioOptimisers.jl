...
A [`SemiDefinitePhylogeny`](@ref) in the constraints also moves the variance to this formulation. The trace ``\\mathrm{Tr}(\\mathbf{\\Sigma} \\mathbf{W})`` has degree one in ``(\\boldsymbol{w}, k)``, and the other formulations have degree two. So under [`MaximumRatio`](@ref) this formulation maximises the excess return per unit of variance, not the Sharpe ratio. The `## The degree of the risk` subsection of [`MaximumRatio`](@ref) states the rule.
