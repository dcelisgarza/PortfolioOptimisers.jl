...
const arg_dict = unique_key_dict(:arg_dict,
                                 ...
                                 :alpha => "`alpha`: Quantile level for the lower tail. The bound is [`Num_SigTailCal`](@ref), so the slot takes the level itself or a [`SignificanceTailCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 :alpha_ltd => "`alpha`: Quantile level for the lower tail.",#
                                 :beta => "`beta`: Quantile level for the upper tail. The bound is [`Num_SigHeadCal`](@ref), so the slot takes the level itself or a [`SignificanceHeadCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 ...
                                 :cal_alg_sig => "`alg`: Calibration rule that computes the significance level from the data the prior result carries. It is run by calling it, so it is either a callable [`AbstractSignificanceCalibrationAlgorithm`](@ref) or a plain function of the same five arguments. The bound is [`Func_SigCal`](@ref), so a deformation rule, or a calibration role, in this field is refused at construction.",#
                                 :cal_alg_def => "`alg`: Calibration rule that computes the deformation parameter from the data the prior result carries. It is run by calling it, so it is either a callable [`AbstractDeformationCalibrationAlgorithm`](@ref) or a plain function of the same five arguments. The bound is [`Func_DefCal`](@ref), so a significance rule, or a calibration role, in this field is refused at construction.",#
                                 ...
                                 :cal_alg_amb_rad => "`alg`: Calibration rule that computes the ambiguity radius from the data the prior result carries. It is run by calling it, so it is either a callable [`AbstractAmbiguityRadiusCalibrationAlgorithm`](@ref) or a plain function of the same five arguments. The bound is [`Func_AmbRadCal`](@ref), so a tail-weight rule, or a calibration role, in this field is refused at construction.",#
                                 :cal_alg_amb_twt => "`alg`: Calibration rule that computes the Esfahani-Kuhn tail weight from the data the prior result carries. It is run by calling it, so it is either a callable [`AbstractAmbiguityTailWeightCalibrationAlgorithm`](@ref) or a plain function of the same five arguments. The bound is [`Func_AmbTwtCal`](@ref), so a radius rule, or a calibration role, in this field is refused at construction.",#
                                 ...
                                 :cal_alg_norm_ceil => "`alg`: Calibration rule that computes the norm ceiling from the data the prior result carries. It is run by calling it, so it is either a callable [`AbstractNormCeilingCalibrationAlgorithm`](@ref) or a plain function of the same five arguments. The bound is [`Func_NormCeilCal`](@ref), so a radius rule, or a calibration role, in this field is refused at construction.",#
                                 ...
                                 :kappa => "`kappa`: Relativistic deformation parameter. The bound is [`Num_DefTailCal`](@ref), so the slot takes the parameter itself or a [`DeformationTailCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 :kappa_a => "`kappa_a`: Relativistic deformation parameter for the lower tail. The bound is [`Num_DefTailCal`](@ref), so the slot takes the parameter itself or a [`DeformationTailCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 :kappa_b => "`kappa_b`: Relativistic deformation parameter for the upper tail. The bound is [`Num_DefHeadCal`](@ref), so the slot takes the parameter itself or a [`DeformationHeadCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 :l_a => "`l_a`: Weight of the tail term in the Esfahani-Kuhn loss of the lower tail. The mean term is not scaled by it. The bound is [`Num_AmbTwtCal`](@ref), so the slot takes the weight itself or an [`AmbiguityTailWeightCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 :r_a => "`r_a`: Radius of the type-1 Wasserstein ambiguity ball of the lower tail. It multiplies a decision variable, so it is not a constant offset. The bound is [`Num_AmbRadCal`](@ref), so the slot takes the radius itself or an [`AmbiguityRadiusCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 :l_b => "`l_b`: Weight of the tail term in the Esfahani-Kuhn loss of the upper tail. The mean term is not scaled by it. The bound is [`Num_AmbTwtCal`](@ref), so the slot takes the weight itself or an [`AmbiguityTailWeightCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 :r_b => "`r_b`: Radius of the type-1 Wasserstein ambiguity ball of the upper tail. It multiplies a decision variable, so it is not a constant offset. The bound is [`Num_AmbRadCal`](@ref), so the slot takes the radius itself or an [`AmbiguityRadiusCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 ...
                                 :l2c => "`l2c`: 2-norm ceiling on the weights — bounds `norm(w, 2) <= l2c * k` (`k` is the budget, `1` for a fully invested portfolio). Smaller `l2c` forces a more evenly spread portfolio. Used as a diversification floor via the reciprocal: `l2c = 1 / sqrt(m)` requires at least `m` effective assets (`inv(norm(w, 2)^2) >= m`). Norm-constraint family with `lpc` and `linfc`. The bound is [`Num_NormCeilCal`](@ref) under the time-dependent wrapper, so the slot takes the ceiling itself or a [`NormCeilingCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 ...
                                 :linfc => "`linfc`: ∞-norm ceiling on the weights — a cap on the largest absolute weight: `norm(w, Inf) <= linfc * k`. So `linfc = 0.2` caps the largest weight at 20% of a fully invested portfolio. Used as a diversification floor via the reciprocal: `linfc = 1 / m` spreads the portfolio across at least `m` assets. Norm-constraint family with `l2c` and `lpc`. The bound is [`Num_NormCeilCal`](@ref) under the time-dependent wrapper, so the slot takes the ceiling itself or a [`NormCeilingCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 :l1 => "`l1`: L1 regularisation coefficient. It is the ambiguity radius of a type-``\\infty`` Wasserstein ground metric, whose dual norm is the 1-norm, so the bound is [`Num_AmbRadCal`](@ref) under the time-dependent wrapper and the slot takes an [`AmbiguityRadiusCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 :l2 => "`l2`: L2 regularisation term(s).",#
                                 :linf => "`linf`: L∞ regularisation coefficient. It is the ambiguity radius of a type-1 Wasserstein ground metric, whose dual norm is the ``\\infty``-norm, so the bound is [`Num_AmbRadCal`](@ref) under the time-dependent wrapper and the slot takes an [`AmbiguityRadiusCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref).",#
                                 :lp => "`lp`: Lp regularisation specification(s).",#
                                 :l2reg_val => "`val`: L2 regularisation penalty coefficient. It is the ambiguity radius of a type-2 Wasserstein ground metric, so the bound is [`Num_AmbRadCal`](@ref) and the slot takes an [`AmbiguityRadiusCalibration`](@ref) that computes it from the prior result. A rule of that role's family is admitted alone, and the slot puts the role on through [`bind_role`](@ref). That reading holds only for the un-squared penalty of [`SOCRiskExpr`](@ref), so [`assert_ambiguity_radius_formulation`](@ref) refuses a rule beside a squared formulation.",#
                                 :l2reg_alg => "`alg`: Second-moment formulation used to express the L2 penalty.",#
                                 :lpreg_p => "`p`: Norm order, `p > 1`.",#
                                 :lpreg_val => "`val`: Penalty coefficient when the estimator is used as a regularisation term (the `lp` field of [`JuMPOptimiser`](@ref)), or the upper bound on the p-norm of the weights when it is used as a norm constraint (the `lpc` field). As a regularisation term it is the ambiguity radius of a type-``q`` Wasserstein ground metric with ``1/p + 1/q = 1``. As a norm constraint it is a ceiling, which is a different quantity. One field therefore carries two readings, so the bound is [`Num_AmbRadNormCeilCal`](@ref), which admits both roles and both rule families, and each of the two routes refuses the role that has no reading on it. A rule alone takes the role of its own family through [`bind_role`](@ref).",#
