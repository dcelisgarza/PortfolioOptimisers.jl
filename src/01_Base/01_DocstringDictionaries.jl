...
const arg_dict = unique_key_dict(:arg_dict,
                                 ...
                                 :pe_ucs => "`pe`: Prior estimator the set fits on the returns it is handed, or `nothing`. With `nothing` the set holds no prior of its own and is calibrated on the prior result it is handed — inside an optimiser, the prior the optimiser is solving on, so its centre is the objective's own — through the prior-result arm of the ucs triple; the returns-data form then refuses by name (ADR 0138). The default, `EmpiricalPrior()`, fits an empirical prior on the returns.",#
                                 :pr => "`pr`: Prior result.",#
                                 :per => "`pr`: Prior estimator or result.",#
                                 ...
                                 :ens_prior => "`ens`: Effective sample size behind the moments, or `nothing`, which every reader takes as `size(X, 1)`. Two producers write it: an entropy-pooling prior writes the effective count of its posterior weights `w`, to which it is bound (see [`forward_prior`](@ref)), and [`EmpiricalPrior`](@ref) writes the number of observations its moments were fitted over when a Scenario Cap `max_scenarios` cuts the rows `X` carries below it (ADR 0138), so a consumer that prices a sample size reads the count behind the moments and not the rows carried.",#
