...
const arg_dict = unique_key_dict(:arg_dict,
                                 ...
                                 :pfcache => "`cache`: Optional partial-fit state. It is `nothing` until [`partial_fit!`](@ref) writes one, and the estimator's read-out verb reads it when the caller gives no data matrix. [`factory`](@ref), [`port_opt_view`](@ref) and [`obs_weights_view`](@ref) carry it unchanged, so an estimator restricted to a subset of assets or observations after a partial fit must be fitted again. Issue #712 asks whether a channel must instead drop or slice it.",
