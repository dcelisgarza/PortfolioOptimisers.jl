...
unique_key_dict!(arg_dict, :arg_dict,
                 ...
                 :b_mip => "`b`: Big-M constant of the MIP formulation. It relaxes the bound on an observation that the model flags as an exceedance. If `nothing`, the model derives the smallest exact constant from the data and the weight bounds with [`mip_big_m`](@ref), and uses `1000` where the model admits no bound on the gross exposure.",#
                 ...
                 :lq_fees => "`lq`: Proportional liquidation fees, which the fee charges when a position leaves the Investable Mask. The carrier is on the complement of the mask, so its entries are the assets that left and not the assets held. A forced exit trades to zero, so the charge is the rate times the absolute previous weight, and it falls on every period beside `l`, `s` and `tn`.",#
                 :flq_fees => "`flq`: Fixed liquidation fees, which the fee charges when a position leaves the Investable Mask. The carrier is on the complement of the mask, as `lq` is. The fee charges the amount once for each entry whose absolute previous weight is not `isapprox` to zero under `kwargs`, and it falls on the clock `fa` names beside `fl` and `fs`.",#
                 ...
                 :kwargs_fee => "`kwargs`: Named tuple of keyword arguments that the fixed fee terms forward to `isapprox`, to decide how near zero a weight must be to count as zero.",#
                 :imsk_fees => "`imsk`: The Investable Mask that a door reduced this fee on, or `nothing` when the fee is a caller's statement over the full universe. Only the door, [`investable_fees_view`](@ref), writes it, and a caller never does. When it is set, `tn`, `l`, `s`, `fl` and `fs` are on `findall(imsk)`, and `lq` and `flq` are on its complement. A door that meets a marked fee a second time returns it unchanged. It would drop or re-slice the carriers of an unmarked fee.",#
                 # Tracking error.
