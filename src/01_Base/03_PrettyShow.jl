...
The default method returns every declared field. A type that hides one of its fields overloads this method for itself and returns the others. The overload receives the instance, so it may hide a field whatever it holds — a partial-fit `cache`, which is running detail rather than configuration — or hide it by what it holds, as the moment estimators hide an opt-in `cvg` that is `nothing`: an opt-in the caller has not taken is not part of the configuration they chose, and a field added to a type this way moves no rendered docstring. That overload is the per-type arm of the field selection, and [`SHOW_NOTHING_FIELDS`](@ref) is the configured arm: [`pretty_show_fields`](@ref) resolves the two, and a per-name entry of the configuration overrides this method in either direction. ADR 0105 records the arms and their precedence.
