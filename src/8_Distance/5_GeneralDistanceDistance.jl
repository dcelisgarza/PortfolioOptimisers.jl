...
# Related
  ...
  - [`Distances.jl`](https://github.com/JuliaStats/Distances.jl)]
"""
function GeneralDistanceDistance(;
...
function distance(
    ...
    dist = distance(
        GeneralDistance(; power = de.power, alg = de.alg),
        ce,
        X;
        dims = dims,
        kwargs...,
...
function distance(de::GeneralDistanceDistance, rho::AbstractMatrix, args...; kwargs...)
    dist =
        distance(GeneralDistance(; power = de.power, alg = de.alg), rho, args...; kwargs...)
    return Distances.pairwise(de.dist, dist, de.args...; de.kwargs...)
end
"""
