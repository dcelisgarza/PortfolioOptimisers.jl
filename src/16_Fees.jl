...
function FeesEstimator(;
    tn::Union{Nothing,<:TurnoverEstimator,<:Turnover} = nothing,
    l::Union{
        Nothing,
        <:AbstractDict,
        <:Pair{<:Any,<:Real},
...
function Fees(;
    ...
    elseif isa(l, AbstractVector)
        @argcheck(
            !isempty(l) && all(x -> x >= zero(x), l),
            AssertionError(
                "`l` must be non-empty and all must be non-negative:\nall(x -> x >= zero(x), l) => $(all(x -> x >= zero(x), l))",
            )
        )
    end
    if isa(s, Real)
    ...
    elseif isa(s, AbstractVector)
        @argcheck(
            !isempty(s) && all(x -> x >= zero(x), s),
            AssertionError(
                "`s` must be non-empty and all must be non-negative:\nall(x -> x >= zero(x), s) => $(all(x -> x >= zero(x), s))",
            )
        )
    end
    if isa(fl, Real)
    ...
    elseif isa(fl, AbstractVector)
        @argcheck(
            !isempty(fl) && all(x -> x >= zero(x), fl),
            AssertionError(
                "`fl` must be non-empty and all must be non-negative:\nall(x -> x >= zero(x), fl) => $(all(x -> x >= zero(x), fl))",
            )
        )
    end
    if isa(fs, Real)
    ...
    elseif isa(fs, AbstractVector)
        @argcheck(
            !isempty(fs) && all(x -> x >= zero(x), fs),
            AssertionError(
                "`fs` must be non-empty and all must be non-negative:\nall(x -> x >= zero(x), fs) => $(all(x -> x >= zero(x), fs))",
            )
        )
    end
...
function calc_asset_fixed_fees(
    w::AbstractVector,
    fees::Real,
    kwargs::NamedTuple,
    op::Function,
