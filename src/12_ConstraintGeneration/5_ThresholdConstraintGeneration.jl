...
function threshold_view(
    t::AbstractVector{<:Union{Nothing,<:BuyInThreshold,<:BuyInThresholdEstimator}},
    i::AbstractVector,
)
...
function threshold_constraints(
    t::BuyInThresholdEstimator,
    sets::AssetSets;
    datatype::DataType = Float64,
    strict::Bool = false,
)
    return BuyInThreshold(;
        val = estimator_to_val(t.val, sets, zero(datatype); strict = strict),
    )
end
...
function threshold_constraints(
    t::AbstractVector{<:Union{Nothing,<:BuyInThresholdEstimator,<:BuyInThreshold}},
    sets::AssetSets;
    kwargs...,
)
