...
function AssetSets(;
    key::Union{Symbol,<:AbstractString} = "nx",
    dict::AbstractDict{<:Union{Symbol,<:AbstractString}},
)
    @argcheck(
        !isempty(dict) && haskey(dict, key),
        AssertionError(
...
function group_to_val!(
    nx::AbstractVector,
    sdict::AbstractDict,
    key::Any,
    val::Real,
    dict::Union{<:AbstractDict,<:Pair{<:Any,<:Real},<:AbstractVector{<:Pair{<:Any,<:Real}}},
    arr::AbstractArray,
    strict::Bool,
)
...
function estimator_to_val!(
    arr::AbstractArray,
    dict::Union{<:AbstractDict,<:AbstractVector{<:Pair{<:Any,<:Real}}},
    sets::AssetSets;
    strict::Bool = false,
)
...
function _parse_equation(lhs, opstr::AbstractString, rhs, datatype::DataType = Float64)
    ...
    lhs_str = join(
        [_format_term(coeff, var) for (coeff, var) in zip(coefficients, variables)],
        " + ",
    )
...
function parse_equation(
    ...
    if isnothing(op)
        error(
            "Equation must contain a valid comparison operator $(join(ops1,", ")) .\n$(eqn)",
        )
    end
    opstr = ops1[op]
...
function parse_equation(
    eqn::AbstractVector{<:Union{<:AbstractString,Expr}};
    ops1::Tuple = ("==", "<=", ">="),
    ops2::Tuple = (:call, :(==), :(<=), :(>=)),
    datatype::DataType = Float64,
)
...
function replace_group_by_assets(
    res::ParsingResult,
    sets::AssetSets,
    bl_flag::Bool = false,
    ...
    if bl_flag && (rho_flag || prior_flag)
        throw(
            ArgumentError(
                "`bl_flag` can only be used if `prior_flag` and `rho_flag` are false.",
            ),
        )
    end
        ...
        if isnothing(m)
            ...
            if isnothing(n) && !rho_flag
                ...
                if !(prior_flag && rho_flag)
                    throw(
                        ArgumentError(
                            "`(a, b)` can only be used for rho_views in entropy pooling.",
                        ),
                    )
                end
                if isnothing(n)
                    throw(
                        ArgumentError(
                            "Correlation views can only be of the form `(a, b)`.",
                        ),
                    )
                end
                asset1 = n.captures[1]
            ...
            if isnothing(n) && !rho_flag
                ...
                if !rho_flag
                    throw(
                        ArgumentError(
                            "`prior(a, b)` can only be used for rho_views in entropy pooling.",
                        ),
                    )
                end
                if isnothing(n)
    ...
    eqn = replace(
        join(string.(coeffs_new) .* "*" .* variables_new, " + "),
        "+ -" => "-",
        "  " => " ",
    )
...
function replace_group_by_assets(
    res::AbstractVector{<:ParsingResult},
    sets::AssetSets,
    args...,
)
    return replace_group_by_assets.(res, sets, args...)
end
function get_linear_constraints(
...
function RiskBudgetResult(; val::AbstractVector{<:Real})
    @argcheck(
        ...
        AssertionError(
            "`val` must be non-empty and all its entries must be non-negative:\n!isempty(val) => $(!isempty(val))\nall(x -> x >= zero(x), val) => $(all(x -> x >= zero(x), val))",
        )
    )
...
function risk_budget_constraints(
    ::Nothing,
    args...;
    N::Real,
    datatype::DataType = Float64,
...
function risk_budget_constraints(
    rb::Union{<:AbstractDict,<:Pair{<:Any,<:Real},<:AbstractVector{<:Pair{<:Any,<:Real}}},
    sets::AssetSets;
    N::Real = length(sets.dict[sets.key]),
    strict::Bool = false,
    datatype::DataType = Float64,
)
...
function RiskBudgetEstimator(;
    ...
    if isa(val, Union{<:AbstractDict,<:AbstractVector})
        ...
        if isa(val, AbstractDict)
            @argcheck(
                all(x -> x >= zero(x), values(val)),
                DomainError("All entries of `val` must be non-negative")
            )
        elseif isa(val, AbstractVector{<:Pair})
            @argcheck(
                all(x -> x >= zero(x), getproperty.(val, :second)),
                DomainError(
                    "The numerical value of all entries of `val` must be non-negative",
                )
            )
        end
    elseif isa(val, Pair)
        @argcheck(
            val.second >= zero(val.second),
            DomainError(
                "The numerical value of `val` must be non-negative:\nval.second => $(val.second)",
            )
        )
    end
...
function asset_sets_matrix(smtx::Union{Symbol,<:AbstractString}, sets::AssetSets)
    ...
    @argcheck(
        length(sets.dict[sets.key]) == length(all_sets),
        AssertionError(
...
function linear_constraints(
    lcs::AbstractVector{<:LinearConstraintEstimator},
    sets::AssetSets;
    datatype::DataType = Float64,
    strict::Bool = false,
    bl_flag::Bool = false,
...
function asset_sets_matrix(
    smtx::AbstractVector{<:Union{Nothing,<:AbstractMatrix,<:AssetSetsMatrixEstimator}},
    sets::AssetSets,
)
    return asset_sets_matrix.(smtx, Ref(sets))
end
function asset_sets_matrix_view(
