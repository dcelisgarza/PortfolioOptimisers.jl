...
const arg_dict = unique_key_dict(:arg_dict,
                                 ...
                                 :ep_tv_evar_zlo => "`zlo`: Lower end of the bracket of the dual variable, as a fraction of the upper end, or `nothing` to take the span [`ep_evar`](@ref) states. The upper end is a proof, so it is not a knob and only the lower one is.",#
                                 ...
                                 :ep_bracket_rlvar_zlo => "`zlo`: Lower end of the bracket of the logarithm of the dual variable, as an offset from the logarithm of the loss range.",#
                                 :ep_bracket_rlvar_zhi => "`zhi`: Upper end of the bracket of the logarithm of the dual variable, as an offset from the logarithm of the loss range.",#
                                 ...
                                 :cal_alpha_sib => "`alpha`: Significance level of the sibling slot, or `nothing` until [`bind_alpha`](@ref) fills it. The two slots travel together, so the per-type resolution resolves `alpha` first and hands the number over.",#
                                 :cal_series => "`series`: The series the estimate is taken over, one of [`ReturnsSeries`](@ref), [`AbsoluteDrawdownSeries`](@ref) and [`RelativeDrawdownSeries`](@ref). It belongs to the slot owner rather than to the rule, so the owner fills it through [`bind_series`](@ref) and that call overwrites whatever the field holds. A drawdown marker pools the per-column drawdown series of `pr.X` in place of its columns. State it only to run the rule outside a measure.",#
                                 :cal_series_rad => "`series`: The series the deformation parameter is read for, one of [`ReturnsSeries`](@ref), [`AbsoluteDrawdownSeries`](@ref) and [`RelativeDrawdownSeries`](@ref). It belongs to the slot owner rather than to the rule, so the owner fills it through [`bind_series`](@ref) and that call overwrites whatever the field holds. The estimate itself is radial under every marker, and a drawdown marker shifts the index it returns. State it only to run the rule outside a measure.",#
                                 :cal_series_scale => "`series`: The series the radius is a distance over, one of [`ReturnsSeries`](@ref), [`AbsoluteDrawdownSeries`](@ref) and [`RelativeDrawdownSeries`](@ref). It belongs to the slot owner rather than to the rule, so the owner fills it through [`bind_series`](@ref) and that call overwrites whatever the field holds. A drawdown marker reads the per-asset dispersion off the drawdown sample of `pr.X`, because `pr.sigma` states the moments of the returns alone. State it only to run the rule outside a measure.",#
                                 :cal_series_twt => "`series`: The series both terms of the ratio are read over, one of [`ReturnsSeries`](@ref), [`AbsoluteDrawdownSeries`](@ref) and [`RelativeDrawdownSeries`](@ref). It belongs to the slot owner rather than to the rule, so the owner fills it through [`bind_series`](@ref) and that call overwrites whatever the field holds. A drawdown marker reads the mean term and the tail term off the drawdown sample of `pr.X` in place of its columns. State it only to run the rule outside a measure.",#
                                 :cal_alg_amb_rad => "`alg`: Calibration rule that computes the ambiguity radius from the data the prior result carries. It is run by calling it, so it is either a callable [`AbstractAmbiguityRadiusCalibrationAlgorithm`](@ref) or a plain function of the same four arguments. The bound is [`Func_AmbRadCal`](@ref), so a tail-weight rule, or a calibration role, in this field is refused at construction.",#
                                 :cal_alg_amb_twt => "`alg`: Calibration rule that computes the Esfahani-Kuhn tail weight from the data the prior result carries. It is run by calling it, so it is either a callable [`AbstractAmbiguityTailWeightCalibrationAlgorithm`](@ref) or a plain function of the same four arguments. The bound is [`Func_AmbTwtCal`](@ref), so a radius rule, or a calibration role, in this field is refused at construction.",#
                                 ...
                                 :r_b => "`r_b`: Radius of the type-1 Wasserstein ambiguity ball of the upper tail. It multiplies a decision variable, so it is not a constant offset. The bound is [`Num_AmbRadCal`](@ref), so the slot takes the radius itself or an [`AmbiguityRadiusCalibration`](@ref) that computes it from the prior result.",#
                                 ...
                                 :l2c => "`l2c`: 2-norm ceiling on the weights — bounds `norm(w, 2) <= l2c * k` (`k` is the budget, `1` for a fully invested portfolio). Smaller `l2c` forces a more evenly spread portfolio. Used as a diversification floor via the reciprocal: `l2c = 1 / sqrt(m)` requires at least `m` effective assets (`inv(norm(w, 2)^2) >= m`). Norm-constraint family with `lpc` and `linfc`. The bound is [`Num_NormCeilCal`](@ref) under the time-dependent wrapper, so the slot takes the ceiling itself or a [`NormCeilingCalibration`](@ref) that computes it from the prior result.",#
                                 ...
                                 :linfc => "`linfc`: ∞-norm ceiling on the weights — a cap on the largest absolute weight: `norm(w, Inf) <= linfc * k`. So `linfc = 0.2` caps the largest weight at 20% of a fully invested portfolio. Used as a diversification floor via the reciprocal: `linfc = 1 / m` spreads the portfolio across at least `m` assets. Norm-constraint family with `l2c` and `lpc`. The bound is [`Num_NormCeilCal`](@ref) under the time-dependent wrapper, so the slot takes the ceiling itself or a [`NormCeilingCalibration`](@ref) that computes it from the prior result.",#
                                 :l1 => "`l1`: L1 regularisation coefficient. It is the ambiguity radius of a type-``\\infty`` Wasserstein ground metric, whose dual norm is the 1-norm, so the bound is [`Num_AmbRadCal`](@ref) under the time-dependent wrapper and the slot takes an [`AmbiguityRadiusCalibration`](@ref) that computes it from the prior result.",#
                                 :l2 => "`l2`: L2 regularisation term(s).",#
                                 :linf => "`linf`: L∞ regularisation coefficient. It is the ambiguity radius of a type-1 Wasserstein ground metric, whose dual norm is the ``\\infty``-norm, so the bound is [`Num_AmbRadCal`](@ref) under the time-dependent wrapper and the slot takes an [`AmbiguityRadiusCalibration`](@ref) that computes it from the prior result.",#
                                 :lp => "`lp`: Lp regularisation specification(s).",#
                                 :l2reg_val => "`val`: L2 regularisation penalty coefficient. It is the ambiguity radius of a type-2 Wasserstein ground metric, so the bound is [`Num_AmbRadCal`](@ref) and the slot takes an [`AmbiguityRadiusCalibration`](@ref) that computes it from the prior result. That reading holds only for the un-squared penalty of [`SOCRiskExpr`](@ref), so [`assert_ambiguity_radius_formulation`](@ref) refuses a rule beside a squared formulation.",#
                                 :l2reg_alg => "`alg`: Second-moment formulation used to express the L2 penalty.",#
                                 :lpreg_p => "`p`: Norm order, `p > 1`.",#
                                 :lpreg_val => "`val`: Penalty coefficient when the estimator is used as a regularisation term (the `lp` field of [`JuMPOptimiser`](@ref)), or the upper bound on the p-norm of the weights when it is used as a norm constraint (the `lpc` field). As a regularisation term it is the ambiguity radius of a type-``q`` Wasserstein ground metric with ``1/p + 1/q = 1``. As a norm constraint it is a ceiling, which is a different quantity. One field therefore carries two readings, so the bound is [`Num_AmbRadNormCeilCal`](@ref), which admits both roles, and each of the two routes refuses the role that has no reading on it.",#
