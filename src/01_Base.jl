...
const arg_dict = Dict(
                      ...
                      :pred_nz => "`nz`: Feature name vector.",#
                      :pred_Z => "`Z`: Feature matrix collapsed onto the fold's synthetic asset (observations × features), or one such matrix per weight vector. Transport only — nothing reads it off this carrier.",#
