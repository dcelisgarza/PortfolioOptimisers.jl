...
const arg_dict = unique_key_dict(:arg_dict,
                                 ...
                                 :rlvar_zpct => "`pct`: Fractional half-width of the grid of relativistic value-at-risk dual variables, centred on the value that attains the prior relativistic value-at-risk. It sets the reach of the grid, which is what decides whether the view lands on its target: the dual variable the posterior attains is not the one the prior attains, and the view reaches its target only where the grid holds it. The width that covers that variable does not shrink as the target nears the prior, because its two ends move in opposite directions, so a width that scales with the size of the move is worse than a constant one.",#
                                 :rlvar_zK => "`K`: Number of points of the grid of relativistic value-at-risk dual variables. Must be odd. It sets the resolution of the grid alone, and the spacing is `2 * pct * zstar / (K - 1)`. Every point is one more binary variable of the mixed-integer program an upper-bound or equality view builds, so raise it when `pct` widens rather than on its own.",#
                                 ...
                                 :ep_rlvar_tgrid => "`t`: Shift variable that attains the prior objective at each point of `z`, one entry per grid point.",#
