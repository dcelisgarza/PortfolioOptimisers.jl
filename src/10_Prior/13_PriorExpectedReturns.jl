...
The adapter folds exactly when its prior does under the library's rule ([`supports_partial_fit`](@ref)): [`partial_fit!`](@ref) forwards to the prior and `mean(me)` reads `prior(pe).mu`. An [`EmpiricalPrior`](@ref) carries its rows as memory rather than folding its moments alone, so under that rule it refits from the rows a host holds — which for the online portfolio selection head is the head's own buffer, held once. That buffer has no bound for an adapter that does not fold ([`rows_needed`](@ref) answers `nothing`): the head keeps every row, and the adapter costs one prior fit over the whole prefix at every step. A caller who wants a window wraps the adapter in a [`WindowedExpectedReturns`](@ref).

The adapter fits the whole prior, so a covariance the prior cannot form makes the mean throw, although `mu` alone is defined. Under the default [`EmpiricalPrior`](@ref), rows that hold a column of constant returns, such as a suspended asset in the head's buffer, throw an `ArgumentError` in the positive-definite repair of the covariance. A caller whose rows can hold such a column gives the prior a covariance without that repair, `PortfolioOptimisersCovariance(; mp = MatrixProcessing(; pdm = nothing))`.
