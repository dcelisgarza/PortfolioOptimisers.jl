...
function StatsBase.cov(ce::SmythBrobyCovariance, X::AbstractMatrix; dims::Int = 1,
                       mean = nothing)
    @smart_assert(dims ∈ (1, 2))
    if dims == 2
        X = transpose(X)
...
struct NormalisedSmythBrobyCovariance{T1 <: SmythBrobyCovarianceAlgorithm,
                                      T2 <: AbstractExpectedReturnsEstimator,
                                      T3 <: StatsBase.CovarianceEstimator,
                                      T4 <: PosDefEstimator, T5 <: Real, T6 <: Real,
                                      T7 <: Real, T8 <: Real, T9 <: Real} <:
       BaseSmythBrobyCovariance
    alg::T1
    me::T2
    ve::T3
...
function NormalisedSmythBrobyCovariance(;
    ...
    @smart_assert(zero(c1) < c1 <= one(c1))
    @smart_assert(zero(c2) < c2 <= one(c2) && c3 > c2)
    return NormalisedSmythBrobyCovariance{typeof(alg), typeof(me), typeof(ve), typeof(pdm),
...
function smythbroby(ce::NormalisedSmythBrobyCovariance{<:SmythBroby0, <:Any, <:Any, <:Any,
                                                       <:Any, <:Any, <:Any, <:Any, <:Any},
                    X::AbstractMatrix)
    T, N = size(X)
    rho = Matrix{eltype(X)}(undef, N, N)
...
function StatsBase.cov(ce::NormalisedSmythBrobyCovariance, X::AbstractMatrix; dims::Int = 1,
                       mean = nothing)
    @smart_assert(dims ∈ (1, 2))
    if dims == 2
        X = transpose(X)
...
export SmythBroby0, SmythBroby1, SmythBroby2, SmythBrobyGerber0, SmythBrobyGerber1,
       SmythBrobyGerber2, SmythBrobyCovariance, NormalisedSmythBrobyCovariance
