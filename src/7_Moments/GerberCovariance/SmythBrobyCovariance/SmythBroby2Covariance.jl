struct SmythBroby2Covariance{T1 <: AbstractExpectedReturnsEstimator,
...
function SmythBroby2Covariance(;
    ...
    return SmythBroby2Covariance{typeof(me), typeof(ve), typeof(pdm), typeof(threshold),
                                 typeof(c1), typeof(c2), typeof(c3), typeof(n)}(me, ve, pdm,
                                                                                threshold,
                                                                                c1, c2, c3,
                                                                                n)
end
function w_moment_factory(ce::SmythBroby2Covariance,
                          w::Union{Nothing, <:AbstractWeights} = nothing)
    return SmythBroby2Covariance(; me = w_moment_factory(ce.me, w),
...
function StatsBase.cor(ce::SmythBroby2Covariance, X::AbstractMatrix; dims::Int = 1,
    ...
    return _smythbroby2(ce, X, mean_vec, std_vec)
end
function StatsBase.cov(ce::SmythBroby2Covariance, X::AbstractMatrix; dims::Int = 1,
                       kwargs...)
    @smart_assert(dims ∈ (1, 2))
    if dims == 2
        X = transpose(X)
    ...
    idx = iszero.(std_vec)
    std_vec[idx] .= eps(eltype(X))
    return _smythbroby2(ce, X, mean_vec, std_vec) .* (std_vec ⊗ std_vec)
end

export SmythBroby2Covariance
