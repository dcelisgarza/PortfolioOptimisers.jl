struct SmythBroby1Covariance{T1 <: AbstractExpectedReturnsEstimator,
...
function SmythBroby1Covariance(;
    ...
    return SmythBroby1Covariance{typeof(me), typeof(ve), typeof(pdm), typeof(threshold),
                                 typeof(c1), typeof(c2), typeof(c3), typeof(n)}(me, ve, pdm,
                                                                                threshold,
                                                                                c1, c2, c3,
                                                                                n)
end
function w_moment_factory(ce::SmythBroby1Covariance,
                          w::Union{Nothing, <:AbstractWeights} = nothing)
    return SmythBroby1Covariance(; me = w_moment_factory(ce.me, w),
...
function StatsBase.cor(ce::SmythBroby1Covariance, X::AbstractMatrix; dims::Int = 1,
    ...
    return _smythbroby1(ce, X, mean_vec, std_vec)
end
function StatsBase.cov(ce::SmythBroby1Covariance, X::AbstractMatrix; dims::Int = 1,
                       kwargs...)
    @smart_assert(dims ∈ (1, 2))
    if dims == 2
        X = transpose(X)
    ...
    idx = iszero.(std_vec)
    std_vec[idx] .= eps(eltype(X))
    return _smythbroby1(ce, X, mean_vec, std_vec) .* (std_vec ⊗ std_vec)
end

export SmythBroby1Covariance
