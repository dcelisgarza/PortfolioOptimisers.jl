struct SmythBrobyGerber0Covariance{T1 <: AbstractExpectedReturnsEstimator,
...
function SmythBrobyGerber0Covariance(;
    ...
    return SmythBrobyGerber0Covariance{typeof(me), typeof(ve), typeof(pdm),
                                       typeof(threshold), typeof(c1), typeof(c2),
                                       typeof(c3), typeof(n)}(me, ve, pdm, threshold, c1,
                                                              c2, c3, n)
end
function w_moment_factory(ce::SmythBrobyGerber0Covariance,
                          w::Union{Nothing, <:AbstractWeights} = nothing)
    return SmythBrobyGerber0Covariance(; me = w_moment_factory(ce.me, w),
                                       ve = w_moment_factory(ce.ve, w), pdm = ce.pdm,
                                       threshold = ce.threshold, c1 = ce.c1, c2 = ce.c2,
                                       c3 = ce.c3, n = ce.n)
end
function _smythbrobygerber0(ce::SmythBrobyGerber0Covariance, X::AbstractMatrix, mean_vec,
                            std_vec)
    T, N = size(X)
...
function StatsBase.cov(ce::SmythBrobyGerber0Covariance, X::AbstractMatrix; dims::Int = 1,
    ...
    idx = iszero.(std_vec)
    std_vec[idx] .= eps(eltype(X))
    return _smythbrobygerber0(ce, X, mean_vec, std_vec) .* (std_vec ⊗ std_vec)
end

export SmythBrobyGerber0Covariance
