struct SmythBrobyGerber2NormalisedCovariance{T1 <: AbstractExpectedReturnsEstimator,
...
function SmythBrobyGerber2NormalisedCovariance(;
    ...
    return SmythBrobyGerber2NormalisedCovariance{typeof(me), typeof(ve), typeof(pdm),
                                                 typeof(threshold), typeof(c1), typeof(c2),
                                                 typeof(c3), typeof(n)}(me, ve, pdm,
                                                                        threshold, c1, c2,
                                                                        c3, n)
end
function w_moment_factory(ce::SmythBrobyGerber2NormalisedCovariance,
                          w::Union{Nothing, <:AbstractWeights} = nothing)
    return SmythBrobyGerber2NormalisedCovariance(; me = w_moment_factory(ce.me, w),
                                                 ve = w_moment_factory(ce.ve, w),
...
function StatsBase.cor(ce::SmythBrobyGerber2NormalisedCovariance, X::AbstractMatrix;
                       dims::Int = 1, kwargs...)
...
function StatsBase.cov(ce::SmythBrobyGerber2NormalisedCovariance, X::AbstractMatrix;
                       dims::Int = 1, kwargs...)
    @smart_assert(dims ∈ (1, 2))
    if dims == 2
        X = transpose(X)
...
export SmythBrobyGerber2NormalisedCovariance
