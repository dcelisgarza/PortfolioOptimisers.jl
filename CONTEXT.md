...
**Online Scheme**
A walk-forward wrapped in `Online`, under which the Fold Loop fits each fold by the online step instead of a refit: the loop warms up once on the first training window and then folds each fold's new observations into one estimator threaded from fold to fold, reading it out where a refit would have run. It is built by its function constructor alone — one per scheme, named `Online` prefixed to the scheme's type name — which takes the scheme's keywords minus its window knob and sets that knob `true`, because a fold cannot un-fold an observation: every nested-prefix scheme steps, and a rolling scheme cannot be written online. A window is the estimator's to declare, through the `Online` wrapper's cap on the prior, never the scheme's. ADR 0140, ADR 0167.
