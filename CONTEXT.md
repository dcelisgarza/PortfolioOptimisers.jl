...
A member of the Online Portfolio Selection family: a struct held on the head's `alg` slot, named by the paper's own name for the algorithm in full words — a leading "Online" dropped because the family says it, a trailing "Optimisation" or "System" dropped because the head says it, "Portfolio" kept where the paper names the portfolio — with the literature's acronym in the docstring and in the roster below, never in the type name. A paper's numbered variant that changes one formula is a typed slot on the rule (`slack`, `formulation`); a variant that changes what is read is the same rule under a different forecast. Every rule is one of three mechanisms: a closed form followed by the projection, an Expert Mixture, or a follow-the-leader over a Sample Selector. The survey's five families are a docs grouping, not a type layer. ADR 0156.
