...
The best comparator sequence under a path-length budget, `max Σ_t log⟨u_t, x_t⟩` over allocations on the resolved weight bounds with `Σ_t ‖u_t − u_{t−1}‖_p ≤ L`, the comparator dynamic regret at a given budget is stated against: one concave programme over the whole panel, an exponential cone per row and a norm cone per step, solved by `budgeted_hindsight_path(rd, L; p, wb, sets, slv)` and returned as a multi-period prediction result of one fold per row, so the regret verb reads it unchanged. At `L = 0` it is the static best constant rebalanced portfolio, at a slack budget the per-period minimiser, and in between neither per-row comparator of a Hindsight Split. ADR 0165.
*Avoid*: a Hindsight Comparator, whose answer is one allocation; a Hindsight Split, whose folds refit; and a programme Allocation Set, which the verb does not honour — it takes weight bounds alone.
