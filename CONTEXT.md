...
*Avoid*: reading the square shape as cosmetic. It is the `z_sq = true` case, so a subproblem is measured against its own neighbourhood structure rather than the universe's; because both sources refit, that structure is always the subproblem's own. This producer is **endogenous** — both sources derive from the returns — and what a graph source buys was measured: a different merge order and different weights from a correlation distance, but identical cuts at `k = 2` and `k = 3`.
