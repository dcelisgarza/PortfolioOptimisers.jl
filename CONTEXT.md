...
**Allocation Set**
The set an Online Portfolio Selection head's every allocation lies in, held on the head's `set` slot: the simplex by default, and otherwise the weight bounds, linear, turnover, variance-ceiling, tracking-error and cardinality constraints the head honours, always under a budget of one. It comes in two kinds — one that every Projection Geometry reaches in closed form and so carries no solver, and one that is a programme and so requires one — so whether a configuration needs a solver is a fact of its type. Cash is an asset of the set with price relative one, never a budget below one. ADR 0159.
