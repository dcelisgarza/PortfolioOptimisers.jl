...
It declares **every axis it carries**, each with its own key: `xkey`/`uxkey` for assets (defaults `"nx"`/`"ux"`), `fkey`/`ufkey` for factors (`"nf"`/`"uf"`). Assets are the *primary* axis — required, and the one a View slices; the factor axis is optional, and a consumer that needs it and does not find it fails at the point of need rather than at construction. A key prefixed with an axis key is a **partition** of that axis (same length as its universe); a key prefixed with the axis's unique key is the partition's distinct entries. Anything else is a plain group, expanded by name and **axis-blind** — which is why factor groups need no machinery of their own, and why a factor constraint naming an asset group degrades to unknown-name warnings rather than to an error.
*Declaring* the axis is what lets a View leave the factor entries alone: an asset index is meaningless on them. Before ADR 0047 a factor universe was carried in the *asset* key, so the exemption had to be maintained by hand, per field, by omitting an annotation.
