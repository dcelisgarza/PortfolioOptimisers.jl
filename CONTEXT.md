...
Expressed by wrapping a per-fold sequence of values — or a callable (a function, or a functor struct that can additionally declare its target and previous-weights needs as inspectable data) computing the value from the fold's Time-Dependent Context — together with the name of the optimiser input it stands in for.
Entry *i* corresponds to fold *i* of the consuming scheme's split enumeration; the machinery imposes no ordering of its own, so where that enumeration is not a timeline (combinatorial splits, randomised paths) keying entries to time is the user's responsibility.
A time-dependent constraint is the *sole source* of its target: the static input must be left at its default. It participates only where folds exist and is inert everywhere else — a plain (fold-less) optimisation, including a meta-optimiser's full-window solve, runs with the target at its default (the folds belong to whichever cross-validation consumes the entries, which must be sized for it; when fold loops nest, the outermost consumes them).
