...
A consumer that needs to *undo* the residual block reads `factor_residual_config(pe)` rather than the estimator's fields. Only the two lifting estimators carry `ve` and `mp.pdm`, and the slot that holds them is bounded by a union that admits far more, so a field access reaches past the type bound. The declaration is total: the two owners report `(; ve, pdm, rsd)`, a wrapper forwards its wrapped estimator's answer, and everything else reports `nothing`.
