...
**Fold Loop**
The one loop every cross-validation entry point runs, `fold_loop`. Per fold it takes the fold's view of the estimator and its data, swaps the Time-Dependent Inputs against the fold's Time-Dependent Context, threads the previous fold's weights when the estimator needs them, and hands the callback one **Fold**. Three parameters carry every difference between the schemes: whether the fold enumeration is a **timeline** (a timeline can thread previous weights and so runs sequentially when it must; k-fold and combinatorial enumerations are not timelines and always run in parallel), the per-fold **view** (only the asset-resampling paths give one), and the per-fold result element type. `assert_unshuffled_folds` is its companion guard, applied once per split: a scheme may leave gaps in a fold's training indices, but it must never reorder them.
*Avoid*: reading a fold loop as per-scheme — the schemes differ in what a fold *does*, not in how folds are run.

**Fold**
The record the Fold Loop hands its callback: the fold's index and the fold count, its already-resolved estimator, its already-viewed data, and its own training and test index vectors. Everything is resolved before the callback sees it — the asset view is taken, the Time-Dependent Inputs are swapped, the previous weights are threaded — so a callback reads the fold and never re-derives it. Distinct from the **Time-Dependent Context**, which is what a *schedule entry* is resolved against, one step earlier and inside the loop.
*Avoid*: indexing the scheme's index vectors from inside a callback — the Fold already carries that fold's own windows.
