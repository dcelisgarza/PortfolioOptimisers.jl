...
A time-dependent constraint participates only where folds exist and is inert everywhere else — a plain (fold-less) optimisation, including a meta-optimiser's full-window solve, runs with the affected input at its static default (the folds belong to whichever cross-validation consumes the entries, which must be sized for it; when fold loops nest, the outermost consumes them).
