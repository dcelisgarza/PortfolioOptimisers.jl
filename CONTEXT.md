...
- **PriceLevelExpectedReturns**: the return that takes the last price to a statistic of a window of price levels — a moving average, an exponential moving average, a spatial median, a window peak or a lagged price — on a typed `alg` slot; the Price Relative Forecast of the Online Portfolio Selection family, reconstructed from returns with the last level at one. ADR 0158.
