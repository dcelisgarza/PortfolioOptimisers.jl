...
**Start Allocation**
The allocation an Online Portfolio Selection head holds during the first period of its recursion, held on the head's `w0` slot: a given vector over the pinned asset names, pinned and viewed with them and projected once onto the Allocation Set in the rule's own Projection Geometry at the first step, or, absent, `1/N` over the whole pinned universe, unlisted names included, whose legs earn a zero return until they list. It reaches the first Online Update as its `w` and that is the whole of its meaning: a rule that reads `w` continues from it, and a rule that does not — a constant rebalanced portfolio, an Expert Mixture — replaces it at that update, so the fund holds it for one period. ADR 0162.
