...
*Avoid*: reading it as the active mask itself, which is observations × assets on the *returns* clock; the span is on the price clock, and the projection between them is where the padding convention is resolved. And a per-window derivation, which reads a delisting straddling a window's end as dead rather than held.
...
**Universe Policy**
A judgement about the data that changes the asset universe: a coverage floor, a redundancy rule, a score threshold. It is always fitted on a training window and replayed by name on every later window, and it is never derived over the whole panel — the *number* may be the caller's, but the *set it selects* would be computed from observations that postdate every fold, which is survivorship bias in its exact definition. Every one of them is an `AbstractAssetSelector`, so it runs at the returns level after both masks are fixed, and `port_opt_view` slices the masks and the Panel Fields out with the column. The ingestion layer offers none of its own: "not enough history" is answered all-or-nothing by the **Coverage Universe** (§3.6) for an estimator that cannot consume a partial column, and by that estimator's own warm-up when it can. See ADR 0130.
