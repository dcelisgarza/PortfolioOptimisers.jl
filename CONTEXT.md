...
One term of the objective's **return expression**. A JuMP optimiser takes one or a vector of them (`ArithmeticReturn`, dot-product; `LogarithmicReturn`, geometric), and the model-global return expression is their **weighted sum** — there is no Scalariser on this side, a deliberate asymmetry with the Risk Measures (ADR 0052). Each term carries its own settings (`JuMPReturnsSettings`, in a `settings` field placed first): its weight in the sum, its own lower bound, whether it enters the expression at all, and which charges are netted off it. So a term may **bind a bound while contributing nothing** to the objective, and a term that is not in return units simply stays out of the sum. A set-bearing term is centred on the Characteristic Vector its Uncertainty Set carries (§3.9, ADR 0050).
