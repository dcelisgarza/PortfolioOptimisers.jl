...
Its experts read their gradient at their own iterate by default, or, under the `BlendPoint` Gradient Point, at the mixture's played blend, the shared gradient the dynamic-regret mixtures are stated on; its weight vector starts at the mixture's own Start Allocation over the experts, `p0`, uniform by default. The mixture over first-order experts at a geometric rate grid under the exponentiated-gradient weighting and the blend point is the constructors `Ader` and `Sword`; over single-asset constant rebalanced portfolios under the Switching Weighting it is `SwitchingPortfolio`. ADR 0165.
