...
**Investable Mask**
The assets an optimisation can trade, derived from a Prior Result and stored nowhere: `true` where `mu` and the diagonal of `sigma` are both finite. A Prior Estimator fits on the **Coverage Universe** (above) and returns a result on the *full* asset universe, in which an asset it could not estimate carries `NaN`. Every Optimisation Estimator that fits a prior derives the mask once at its entry, before it clusters, samples or solves; reduces the prior and every constraint the caller stated to it; runs whole on the investable universe; and expands the solved weights back into a zero vector of the full length. Its result carries the mask beside the reduced objects it ran on, because the reduced Prior Result can no longer yield it, and the mask is what tells a zero weight from a refusal. An Optimisation Estimator that fits no prior derives no mask and weights every asset it is given. It is `nothing` when every asset is investable, and that sentinel is what skips both halves. See ADR 0115.
