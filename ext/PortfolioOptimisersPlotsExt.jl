...
function PortfolioOptimisers.plot_ptf_cumulative_returns(
    w::AbstractArray,
    X::AbstractMatrix,
...
function PortfolioOptimisers.plot_asset_cumulative_returns(
    w::AbstractVector,
    X::AbstractMatrix,
    fees::Union{Nothing,<:Fees} = nothing;
    ts::AbstractVector = 1:size(X, 1),
    ...
    legend_kwargs::NamedTuple = (; position = :auto),
    ekwargs...,
)
...
function PortfolioOptimisers.plot_composition(
...
function PortfolioOptimisers.plot_risk_contribution(
    r::PortfolioOptimisers.AbstractBaseRiskMeasure,
    w::AbstractVector,
    X::Union{<:AbstractMatrix,<:PortfolioOptimisers.AbstractPriorResult},
    fees::Union{Nothing,<:Fees} = nothing;
    nx::AbstractVector = 1:length(w),
    N::Union{Nothing,<:Real} = nothing,
...
function PortfolioOptimisers.plot_stacked_bar_composition(
    w::Union{<:AbstractVector{<:Real},<:AbstractVector{<:AbstractVector}},
...
function PortfolioOptimisers.plot_stacked_area_composition(
    w::Union{<:AbstractVector{<:Real},<:AbstractVector{<:AbstractVector}},
    nx::AbstractVector = 1:size(w, 1);
    kwargs::NamedTuple = (;
        xlabel = "Portfolios",
    ...
    return areaplot(
        transpose(w);
        xticks = (1:M, 1:M),
        label = permutedims(nx),
        kwargs...,
        ekwargs...,
    )
end
function PortfolioOptimisers.plot_dendrogram(
    clr::PortfolioOptimisers.AbstractClusteringResult,
    ...
    dend1 = plot(
        clr.clustering;
        normalize = false,
        ylim = extrema(clr.clustering.heights),
        xticks = (1:N, nx),
        dend_kwargs...,
    )
...
function PortfolioOptimisers.plot_clusters(
    pe::Union{
    ...
    dend2 = plot(
        ...
        yflip = true,
        xlim = extrema(clr.clustering.heights),
        dend2_kwargs...,
    )
    for (i, cl) in pairs(cls)
        a = [findfirst(x -> x == c, clr.clustering.order) for c in cl]
...
function PortfolioOptimisers.plot_drawdowns(
    w::AbstractArray,
    X::AbstractMatrix,
    ...
    l_kwargs::NamedTuple = (; linewidth = 2, legend = :bottomleft),
    ...
    f_kwargs::NamedTuple = (; size = (750, ceil(Integer, 750 / 1.618))),
    ekwargs...,
)
    ...
    risks =
        100 * if !compound
            [
                ...
                -RelativisticDrawdownatRisk(; slv = slv, alpha = alpha, kappa = kappa)(
                    copy(ret),
                ),
            ...
            [
                -RelativeAverageDrawdown(; w = rw)(copy(ret)),
                -RelativeUlcerIndex()(copy(ret)),
...
function PortfolioOptimisers.plot_measures(
    w::Union{<:AbstractVector{<:Real},<:AbstractVector{<:AbstractVector}},
    ...
    z::Union{Nothing,<:PortfolioOptimisers.AbstractBaseRiskMeasure} = nothing,
...
function PortfolioOptimisers.plot_histogram(
    w::AbstractArray,
    X::AbstractMatrix,
    slv::Union{<:Solver,<:AbstractVector{<:Solver}},
    fees::Union{Nothing,<:Fees} = nothing;
    ...
    risks = (
        ...
        -RelativisticValueatRisk(; w = rw, slv = slv, alpha = alpha, kappa = kappa)(
            copy(ret),
        ),
        mir,
    )
