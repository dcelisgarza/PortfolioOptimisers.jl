...
| A private name's promotion to `public` | [ADR 0154](docs/adr/0154-a-private-abstract-type-earns-a-public-declaration-when-its-interfaces-section-names-the-seam.md) — an existing `# Interfaces` docstring section is the only route | a new census file parallel to `test/test_43_exported_abstract_type_census.jl`, not yet created — its number is picked at infra-ticket time, against `origin/dev` |
...
| `test/test_47_alias_and_module_census.jl` | an acronym alias of `src/23_Aliases.jl` IS the binding its docstring names, a factory alias of that file EQUALS the long form its sentence names, and `src/PortfolioOptimisers.jl` `include`s every other file under `src/` exactly once, in the order of numeric prefixes that are unique within each directory of `src/`, `ext/` and `docs/src/api/` | run the file |
