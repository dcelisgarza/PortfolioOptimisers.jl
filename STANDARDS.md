...
| A file's numeric prefix, or the `include` list of `src/PortfolioOptimisers.jl` | [`.github/instructions/julia-source-code.instructions.md`](.github/instructions/julia-source-code.instructions.md) § *Code Organization* | `test/test_47_alias_and_module_census.jl` |
...
| An `@docs` entry's placement in the mirrored public/private API trees ADR 0128 decides | [ADR 0128](docs/adr/0128-the-docs-split-into-mirrored-public-and-private-api-trees-and-a-census-gates-placement.md) | a new census file parallel to `test/test_43_exported_abstract_type_census.jl`, not yet created — its test_4x number is picked at migration time, against `origin/dev` |
...
| `test/test_47_alias_and_module_census.jl` | an acronym alias of `src/27_Aliases.jl` IS the binding its docstring names, a factory alias of that file EQUALS the long form its sentence names, and `src/PortfolioOptimisers.jl` `include`s every other file under `src/` exactly once, in the order of numeric prefixes that are unique within each directory of `src/`, `ext/` and `docs/src/api/` | run the file |
