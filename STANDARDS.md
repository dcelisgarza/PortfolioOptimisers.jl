...
| The prose of a Literate source under `examples/` or `user_guide/`: the `unslop` rules, the dash, the voice, the vocabulary, a narrated check, the paragraph, and the page's furniture | [`.github/instructions/julia-literate-prose.instructions.md`](.github/instructions/julia-literate-prose.instructions.md) | none yet — the per-page census of map #1218 is built by #1220; when it lands, every counted rule of a page is at or below its row in a generated baseline under `code_health/`, a page with no row fails, and the rules a parser cannot read hold by review |
| A glossary term of `CONTEXT.md`, a contributor's word for a mechanism, or the section sign `§`, inside the prose of a Literate source | [`.github/instructions/julia-literate-prose.instructions.md`](.github/instructions/julia-literate-prose.instructions.md) § *A page names a type by its identifier and a concept in plain words* | none yet — #1220 builds it; the capitalised multi-word terms of `CONTEXT.md`, `seam`, `carrier`, `read-out`, `to the bit`, `refused by name` and `§` are then at or below the page's row |
| A page that states the outcome of a comparison as proved instead of printing its number | [`.github/instructions/julia-literate-prose.instructions.md`](.github/instructions/julia-literate-prose.instructions.md) § *A check is a number the reader reads, never a verdict* | none yet — #1220 builds it; `to the bit`, `by construction`, `honest`, `the identity` and `agrees` are then at or below the page's row |
