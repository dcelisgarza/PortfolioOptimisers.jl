                   ...
                   Section("Moment estimation",
                           [Section("Expected returns",
                            ...
                            Section("Incremental fit",
                                    [Prose("An estimator whose statistic has an exact update from its running state plus one new observation folds observations into that state with [`partial_fit!`](@ref), and its read-out verb answers from the state without reading the sample again. The state lives in the estimator's `cache` field, and ADR 0106 records why it is the one result an estimator holds. The `FullMoment` arm of [`Coskewness`](@ref) and of [`Cokurtosis`](@ref) takes part; the `SemiMoment` arm does not, because it clips against a centre that a new observation moves."),
                                     Cap(:partial_fit!)])]),
