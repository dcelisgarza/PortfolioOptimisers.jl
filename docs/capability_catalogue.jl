...
const CATALOGUE = [Section("Core abstractions",
                   ...
                   Section("Risk measures",
                           [Prose("`PortfolioOptimisers.jl` provides a wide range of risk measures. These are broadly categorised into two types based on the type of optimisations that support them."),
                            ...
                            Section("Calibration",
                                    [Prose("A tail probability and a deformation parameter also take a **rule** in place of the number. The rule goes inside the role that names the end of the distribution the slot addresses, and the role is resolved against the optimisation's own prior, so the quantity refits per cross-validation fold and per meta-optimiser subset where a stated number holds still. The role carries the rule in its `alg` field, and that field takes a callable estimator or a plain function of `(key, pr, w, slv)`. Four rules ship: two compute a significance level, and two compute a deformation parameter. Forty slots across twenty-seven risk measures and weight builders take one: every `alpha` and `beta`, and every `kappa`, `kappa_a` and `kappa_b`. The two inner integration bounds of the tail-Gini family, `alpha_i` and `beta_i`, are starting points rather than quantities to estimate, so they keep their numbers and the joint `0 < alpha_i < alpha < 1` bound is checked against the calibrated `alpha` at fold time."),
                                     Cap(:SignificanceTailCalibration),
                                     Cap(:SignificanceHeadCalibration),
                                     Cap(:DeformationTailCalibration),
                                     Cap(:DeformationHeadCalibration),
                                     Prose("A significance rule reads the sample length, and reads the effective observation weights where the count it states is a count of observations. A deformation rule reads the probability of its own end, which travels to it through `bind_alpha`. One spends a stated entropy budget on the sample length, and the other reads the shape of the sample's own tail and returns the reciprocal of its index, which is a different number at each end of a skewed sample."),
                                     Cap(:ScenarioCount), Cap(:RateSignificance),
                                     Cap(:EntropyBudget), Cap(:HillTailDecay),
                                     Prose("An ambiguity radius takes a rule on the same terms. It names no end of the distribution, so it carries one role rather than two, and it reaches the four regularisation coefficients of [`JuMPOptimiser`](@ref) as well as the two distributionally robust risk measures. Four rules ship, and all four compute a radius. Two shrink the ball at the square-root rate of the sample length, and the third shrinks it at the rate the number of assets sets, which is far slower over a wide universe. The tail weight of an Esfahani-Kuhn loss carries a role and a family of its own, and no rule of that family ships: state a plain function of `(key, pr, w, slv)` for it."),
                                     Cap(:AmbiguityRadiusCalibration),
                                     Cap(:AmbiguityTailWeightCalibration),
