                                              ...
                                              Group(Cap(:NetworkEstimator;
                                                    ...
                                                    [Cap(:KruskalTree, :BoruvkaTree,
                                                     ...
                                                     Group("Triangulated Maximally Filtered Graph with various similarity matrix estimators",
                                                           [Prose("Any member of the similarity matrix family: [`MaximumDistanceSimilarity`](@ref), [`ExponentialSimilarity`](@ref), [`GeneralExponentialSimilarity`](@ref), [`ComplementSimilarity`](@ref) or [`AngularSimilarity`](@ref).")])]),
                                         ...
                                         [Prose("A feature prior attaches an `assets × features` matrix to the prior it wraps, without touching a single moment, so any prior becomes a source for [`FeatureDistance`](@ref). The matrix comes from a feature matrix estimator."),
                                          ...
                                          Group(Cap(:PhylogenyFeatures,
                                                ...
                                                [Prose("Reuses a square `assets × assets` phylogeny or adjacency matrix as features, so the distance measures neighbourhood overlap. The only producer whose feature axis *is* the asset axis; its source is always an estimator, so every fold and subproblem refits the graph on its own universe. Exogenous square structure travels on [`ReturnsResult`](@ref) instead."),
                                                 Cap(:BinaryNeighbourhood),
                                                 Group(Cap(:GradedNeighbourhood),
                                                       [Prose("Keeps the step count the binary variant clamps away, scoring each pair by how far apart it sits. `decay` shapes the fall-off and the source's `n` truncates it -- two knobs, deliberately separate, because an exponential never reaches zero. No decay emits zero inside the budget, so a zero entry means unreachable and nothing else."),
                                                        Group(Cap(:separation_decay,
                                                              ...
                                                              [Prose("The argument is a *real* separation, so one family serves a hop count and any continuous separation alike. The contract -- `f(0) > 0` and maximal, monotone non-increasing, never assumed to reach zero -- is probed by a fail-safe fallback that the shipped members opt out of."),
                                                               Cap(:LinearDecay),
