...
The two functions `fmt1` and `fmt2` format the columns of the two tables that the example prints.
...
!!! tip "Using your own data"
    The file above is a gzipped CSV with a `Date` column and one column per asset, and any price history in that shape works. To use live data, download it with `YFinance.jl` and build a `TimeArray`:
