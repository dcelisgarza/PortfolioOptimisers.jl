...
The first algorithm set of ADR 0156: the two benchmark rules, exponentiated gradient, the online Newton step, passive aggressive mean reversion with its three slack rules, the passive-aggressive step toward a Price Relative Forecast that the moving-average and robust-median reversions share, and the Expert Mixture whose wealth-weighted form over sampled constant rebalanced portfolios is the universal portfolio.
