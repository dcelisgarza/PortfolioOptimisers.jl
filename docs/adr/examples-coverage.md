...
| ✅ | Time-dependent constraints — `TimeDependent` schedules vs callables vs static under every CV scheme, `TimeDependentContext`, `PreviousWeightsFunction`, sole-source/fold-count validation, inertness outside fold loops (ADR 0030) | `examples/5_validation_tuning/04_Time_Dependent_Constraints.jl` |
