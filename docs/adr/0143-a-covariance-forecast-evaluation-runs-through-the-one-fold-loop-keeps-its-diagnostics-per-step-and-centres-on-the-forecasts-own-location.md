...
a carrier, and an `IndexWalkForward` or `DateWalkForward`. Per fold the callback reads `Σ̂` off
the fold's estimator — `cov(est, X_train)` or `prior(pe, rd_train).sigma` under a refit, `cov(est)`
or `prior(pe).sigma` under `OnlineStep()` — and runs the per-step kernel on the test rows. The
...
of them. It lives in `src/20_Optimisation/02_CrossValidation/13_CovarianceForecastEvaluation.jl`,
after the loop it runs through. It owes the loop three small methods:
`partial_fit!(pe::AbstractPriorEstimator, rd)`, which is `fold_prior`;
`partial_fit!(ce::AbstractCovarianceEstimator, rd)`; and `is_time_dependent` for a non-optimiser.
...
family as fact 5 lists: `mean(ce.me, X)` or `state.mu`; the weighted sample mean or `state.mu`;
`state.location`, or zero when `centred = true`; the inner `ce`'s answer for a composite or a
transform; `mu` for a prior; and the window's own sample mean for an estimator that keeps no
location, which under `Online` is the buffer. There is no `demean` flag: the location is the
estimator's, not the caller's, and an estimator that assumed a zero mean is evaluated at zero.

**The diagnostics are the reference's five and three more.** Per step and free of `w`: the
Mahalanobis ratio `tr(Σ̂⁻¹S) / (N h)`, the diagonal ratio per asset `S_ii / (h Σ̂_ii)` — kept as
`M × N`, where the reference keeps the mean over assets — the whole-matrix QLIKE
`h log|Σ̂| + tr(Σ̂⁻¹S)` and the Frobenius loss `‖S/h − Σ̂‖²_F`, the two losses #864 proved robust to
the proxy. Per step and per test portfolio: the standardised return `b_t = w'R / √(h w'Σ̂w)` and
the portfolio QLIKE. Over the run: the mean, median and quantiles of each ratio, the Gaussian band
`1 ± z_{α/2} √(2 / (N Σh))` on the mean ratio and `√(2 / Σh)` on the diagonal and portfolio ratios,
the bias statistic `B = std(b_t)` per portfolio with its cross-portfolio quantiles, the mean losses,
and the exceedance rate of `N h m_t` against `χ²_{Nh}` at `(0.95, 0.99)`. The kurtosis correction
to the band is stated in the docstring and not computed.
...
`GeneralCovariance`, whose fold is the sum-of-blocks Chan merge, and to the tolerance ADR 0137
measured otherwise. The rolling identity is `Online(est; max_history = w)` against
`expand_train = false`, as ADR 0140 states it. The reference is the oracle for `HorizonReturn()`
on a fixture whose location is zero, or with the centring term asserted as the difference.
...
- `partial_fit!` gains a carrier arity on the prior and the covariance estimator, and
  `is_time_dependent` a method for a non-optimiser; `fold_loop` is otherwise untouched.
...
- The reference's raw ratios differ from the library's by the centring term; a parity test
  states it.
- The build is one ticket on map #861; the plots, the optimiser-solved test portfolio, a
  `Pipeline` in the estimator slot, and the covariance metrics as search scorers are fog there.
