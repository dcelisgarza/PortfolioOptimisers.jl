...
| 0081 | [The docstring standard states the model a docstring builds](0081-the-docstring-standard-states-the-model-it-builds.md) | 1 · 2026-08-22 | `# JuMP formulation` names every row a body registers, and `## Relaxation` states the bound when the encoding is not exact. `# Algorithm` holds a procedure as numbered steps, and a selector tag carries neither section. A field description interpolates when more than one field uses it. The Scope widens to `ext/`. The gate reads the manifest's `swept` flag: `# JuMP formulation` by a per-unit presence rule triggered on a body that registers a row, and `# Algorithm` by a per-file floor the row records. |
