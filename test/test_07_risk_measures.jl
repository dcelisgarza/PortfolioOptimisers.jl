...
@testset "X at Risk" begin
    rs = [(ValueatRisk(; alpha = 1e-6), ValueatRisk(; alpha = 1e-6, w = wt)),
          ...
          (EntropicValueatRiskRange(; alpha = 1 - 1e-6, beta = 1 - 1e-6, slv = slv),
           EntropicValueatRiskRange(; alpha = 1 - 1e-6, beta = 1 - 1e-6, w = wt, slv = slv)),
...
@testset "Expected risk" begin
    ...
    @test isapprox(expected_risk(RelativeAverageDrawdown(), w, rd.X), 0.05118499953858111)
    ...
    @test isapprox(expected_risk(RelativeMaximumDrawdown(), w, rd.X), 0.14712227931904298)
