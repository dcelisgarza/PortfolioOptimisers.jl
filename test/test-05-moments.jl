...
@testset "Expected Returns" begin
    mes = [ShrunkExpectedReturns(; alg = JamesStein()),
           ...
           ShrunkExpectedReturns(;
                                 alg = BodnarOkhrinParolya(; target = VolatilityWeighted())),
           ShrunkExpectedReturns(; alg = BodnarOkhrinParolya(; target = MeanSquareError())),
           ShrunkExpectedReturns(; alg = BodnarOkhrinParolya(),
                                 me = SimpleExpectedReturns(; w = ew)),
    ...
    ces = [Covariance(; alg = Full()),
           Covariance(; alg = Full(), me = SimpleExpectedReturns(; w = ew),
                      ce = GeneralWeightedCovariance(;
                                                     ...
                                                     w = ew)),
           Covariance(; alg = Full(),
                      ce = GeneralWeightedCovariance(; ce = AnalyticalNonlinearShrinkage())),
           Covariance(; alg = Semi()),
           ...
           GerberCovariance(;
                            alg = NormalisedGerber0(; me = SimpleExpectedReturns(; w = ew)),
                            ve = SimpleVariance(; me = SimpleExpectedReturns(; w = ew),
                                                corrected = false, w = ew)),
           ...
           SmythBrobyCovariance(; alg = SmythBroby0(), me = SimpleExpectedReturns(; w = ew),
                                ve = SimpleVariance(; me = SimpleExpectedReturns(; w = ew),
                                                    corrected = false, w = ew)),
...
ces = [Covariance(; alg = Full()), SpearmanCovariance(), KendallCovariance(),
       MutualInfoCovariance(), DistanceCovariance(), LTDCovariance(), GerberCovariance(),
       SmythBrobyCovariance()]
des = [Distance(; alg = CanonicalDistance()), DistanceDistance(; alg = CanonicalDistance())]
desg = [GeneralDistance(; alg = CanonicalDistance()),
        GeneralDistanceDistance(; alg = CanonicalDistance())]
for ce in ces, (de, deg) in zip(des, desg)
    cei = PortfolioOptimisersCovariance(; ce = ce)
    ...
    #=
        #######
        dist1 = distance(de, ce, transpose(rd.X); dims = 2)
                dist2 = if isa(ce, MutualInfoCovariance)
                    distance(Distance(;
                                      alg = VariationInfoDistance(; bins = ce.bins,
                                                                  normalise = ce.normalise)),
                             cov(ce, rd.X), rd.X)
                elseif isa(ce, DistanceCovariance)
                    distance(Distance(; alg = CorrelationDistance(;)), cov(ce, rd.X), rd.X)
                elseif isa(ce, LTDCovariance)
                    distance(Distance(; alg = LogDistance()), cov(ce, rd.X), rd.X)
                else
                    distance(de, cov(ce, rd.X), rd.X)
                end
                dist3 = distance(deg, ce, transpose(rd.X); dims = 2)
                dist4 = if isa(ce, MutualInfoCovariance)
                    distance(GeneralDistance(;
                                             alg = VariationInfoDistance(; bins = ce.bins,
                                                                         normalise = ce.normalise)),
                             cov(ce, rd.X), rd.X)
                elseif isa(ce, DistanceCovariance)
                    distance(GeneralDistance(; alg = CorrelationDistance(;)), cov(ce, rd.X), rd.X)
                elseif isa(ce, LTDCovariance)
