...
@testset "Public declaration census: an `# Interfaces` section and a `public`/`export` declaration gate each other (ADR 0154)" begin
    ...
    promotion_debt = Set([:ARCHBootstrapSet, :AbstractAmbiguityRadiusCalibrationAlgorithm,
                          ...
                          :CustomExpectedReturnsValueAlgorithm, :DynamicAbstractWeights,
    ...
    =#
    verb_debt = Set([:_denoise!, :_optimise, :admits, :bin_width, :bootstrap_indices,
                     :calc_num_bins, :compute_pooling, :cs_weights_initial,
                     :cs_weights_refine, :fold_inactive!, :forecast_target_history,
                     :gap_return, :get_node_property, :get_observation_weights, :k_ucs,
                     :matrix_processing_algorithm, :matrix_processing_algorithm!,
                     :merge_states, :needs_previous_weights, :needs_second_pass,
                     :norm_factor, :opt_weight_bounds, :panel_field_axes,
                     :panel_field_labels, :panel_field_stack!, :panel_field_view,
                     :panel_fill, :panel_input_field, :panel_input_is_static,
                     :panel_resolve, :predict_realised_vols, :reads_prior_result,
