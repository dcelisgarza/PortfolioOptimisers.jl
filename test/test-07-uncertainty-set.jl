...
df = CSV.read(joinpath(@__DIR__, "assets/Ellipse-Uncertainty-Sets.csv"), DataFrame)
uesigma_t = CSV.read(joinpath(@__DIR__, "assets/Ellipse-Uncertainty-Sets-Sigma.csv"),
                     DataFrame)
...
res3 = isapprox([mu1; sigma1], df[!, i]; rtol = rtol)
if !res3
    println("Dataframe iteration $i failed")
    find_tol([mu1; sigma1], df[!, i]; name1 = :sigma1, name2 = :sigma2)
end
...
i = PortfolioOptimisers.fourth_moment_index_factory(floor(Int, sqrt(size(ucrs1.sigma, 1))),
                                                    [3, 1])
