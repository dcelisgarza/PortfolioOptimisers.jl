...
for r in rs, obj in objs, ret in rets
    ...
    rtol = if i == 22 && Sys.islinux()
        1e-2
    elseif i in
           (4, 10, 22, 76, 86, 91, 92, 96, 97, 99, 101, 103, 105, 133, 135, 141, 148, 154,
            175, 184, 196, 252)
        5e-5
    elseif i in
           (6, 28, 36, 38, 40, 46, 52, 93, 108, 126, 139, 163, 165, 167, 177, 179, 192, 204,
            214, 216, 254)
        5e-6
    elseif i in (18, 157, 158, 174, 228)
        5e-4
    elseif i in (16, 48, 58, 88, 90, 94, 98, 134, 140, 159, 176)
        1e-5
    ...
    if isa(obj, MaximumRatio)
        ...
        opt1 = JuMPOptimiser(; pe = pr, slv = slv, ret = bounds_returns_estimator(ret, rt))
        mr = MeanRisk(; r = r, opt = opt1)
        res = optimise(mr, rd)
        rt1 = expected_return(ret, res.w, pr)
        @test rt1 >= rt || abs(rt1 - rt) < 1e-10
        mr = MeanRisk(; r = bounds_risk_measure(r, rk), obj = MaximumReturn(), opt = opt)
        res = optimise(mr, rd)
...
@testset "Tracking" begin
    ...
    opt = JuMPOptimiser(; pe = pr, slv = slv,
                        te = TrackingError(; tr = WeightsTracking(; w = w0), err = 2e-3))
    mre = MeanRisk(; obj = MinimumRisk(), opt = opt)
    res = optimise(mre)
