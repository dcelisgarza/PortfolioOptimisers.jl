@safetestset "Prior" begin
    ...
    @testset "High Order Prior" begin
        ...
        @test isapprox(
            pr2.kt,
            cokurtosis(Cokurtosis(; alg = Full()), pr2.X; mean = transpose(pr2.mu)),
        )
        @test (pr2.sk, pr2.V) ==
    ...
    @testset "Vanilla and Bayesian Black Litterman" begin
        ...
        pes = [
            ...
            BayesianBlackLittermanPrior(;
                pe = FactorPrior(; pe = EmpiricalPrior(;)),
                sets = fsets,
                tau = 1 / size(rd.X, 1),
                views = LinearConstraintEstimator(;
                    val = ["MTUM == 0.0001", "QUAL - USMV == -0.0003"],
                ),
            ),
            BlackLittermanPrior(;
                sets = sets,
    ...
    @testset "Factor Black Litterman" begin
        df =
            CSV.read(joinpath(@__DIR__, "./assets/FactorBlackLitterman1.csv.gz"), DataFrame)
        ...
        df =
            CSV.read(joinpath(@__DIR__, "./assets/FactorBlackLitterman2.csv.gz"), DataFrame)
        pe = FactorBlackLittermanPrior(;
            pe = EmpiricalPrior(;),
            sets = fsets,
            l = 2,
    ...
    @testset "Augmented Black Litterman" begin
        df = CSV.read(
            joinpath(@__DIR__, "./assets/AugmentedBlackLitterman.csv.gz"),
            DataFrame,
        )
        pes = [
    ...
    @testset "Entropy pooling" begin
        ...
        @test isapprox(
            pr.w,
            prior(EntropyPoolingPrior(; sets = sets, opt = jopt, mu_views = mu_views), rd).w,
            rtol = 5e-6,
        )
        ...
        @test isapprox(
            pr.w,
            prior(
                EntropyPoolingPrior(; sets = sets, opt = jopt, var_views = var_views),
                rd,
            ).w,
        )
        ...
        @test isapprox(
            pr.w,
            prior(
                EntropyPoolingPrior(; sets = sets, var_views = var_views, opt = jopt),
                rd,
            ).w,
        )
        ...
        pr = prior(
            EntropyPoolingPrior(; sets = sets, var_alpha = 0.07, var_views = var_views),
            rd,
        )
        @test isapprox(
        ...
        @test isapprox(
            pr.w,
            prior(
                EntropyPoolingPrior(; sets = sets, sigma_views = sigma_views, opt = jopt),
                rd,
            ).w,
            rtol = 1e-2,
        )
        ...
        @test isapprox(
            Skewness(; w = pr.w, ve = SimpleVariance(; w = pr.w))(
                [1],
                reshape(pr.X[:, 1], :, 1),
            ),
            2 * Skewness()([1], reshape(pr0.X[:, 1], :, 1)),
            rtol = 2e-3,
        )
        @test isapprox(
            pr.w,
            prior(EntropyPoolingPrior(; sets = sets, opt = jopt, sk_views = sk_views), rd).w,
            rtol = 5e-3,
        )
        ...
        @test isapprox(
            pr.w,
            prior(EntropyPoolingPrior(; sets = sets, opt = jopt, kt_views = kt_views), rd).w,
            rtol = 1e-2,
        )
        ...
        @test isapprox(
            pr.w,
            prior(
                EntropyPoolingPrior(; sets = sets, opt = jopt, rho_views = rho_views),
                rd,
            ).w,
            rtol = 1e-2,
        )
        ...
        pr = prior(EntropyPoolingPrior(; sets = sets, rho_views = rho_views), rd)
        @test isapprox(
            cov2cor(pr.sigma)[1, end],
            cov2cor(pr0.sigma)[1, end] * 0.94,
            rtol = 5e-6,
        )
        @test isapprox(
            pr.w,
            ...
            HighOrderPriorEstimator(;
                pe = EntropyPoolingPrior(;
                    ...
                    mu_views = LinearConstraintEstimator(;
                        val = ["AAPL<=0.75*prior(AAPL)", "XOM >= 0.4*prior(XOM)"],
                    ),
        ...
        @test isapprox(
            ConditionalValueatRisk(; w = pr.w)(rd.X[:, 1]),
            ConditionalValueatRisk(;)(rd.X[:, 1]) * 1.37,
            rtol = 1e-6,
        )
