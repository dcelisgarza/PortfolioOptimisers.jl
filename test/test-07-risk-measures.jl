@safetestset "Risk measures" begin
    using PortfolioOptimisers,
        Test, DataFrames, TimeSeries, CSV, Clarabel, StatsBase, LinearAlgebra
    ...
    @testset "X at Risk" begin
        rs = [
            (ValueatRisk(; alpha = 1e-6), ValueatRisk(; alpha = 1e-6, w = wt)),
    ...
    @testset "Expected risk" begin
        ...
        @test isapprox(
            expected_risk(SquareRootKurtosis(; alg = Semi()), w, rd.X),
        ...
        @test isapprox(
            expected_risk(SquareRootKurtosis(; mu = dot(w, pr.mu)), w, rd.X),
            expected_risk(SquareRootKurtosis(;), w, rd.X),
        )
        @test isapprox(
            expected_risk(SquareRootKurtosis(; w = wt), w, rd.X),
            expected_risk(SquareRootKurtosis(;), w, rd.X),
        )
        @test isapprox(
            expected_risk(
        ...
        @test isapprox(
            ...
            sqrt(
                expected_risk(
                    LowOrderMoment(;
                        alg = LowOrderDeviation(;
                            alg = SecondLowerMoment(; alg = QuadRiskExpr()),
                        ),
                    ),
                    w,
                    rd.X,
        ...
        @test isapprox(
            ...
            sqrt(
                expected_risk(
                    LowOrderMoment(;
                        alg = LowOrderDeviation(;
                            alg = SecondCentralMoment(; alg = QuadRiskExpr()),
                        ),
                    ),
                    w,
                    rd.X,
        ...
        @test isapprox(
            expected_risk(LowOrderMoment(; alg = MeanAbsoluteDeviation()), w, rd.X),
            0.009807328313217291,
        )
        @test isapprox(
            expected_risk(LowOrderMoment(; alg = MeanAbsoluteDeviation()), w, rd.X),
            expected_risk(LowOrderMoment(; w = wt, alg = MeanAbsoluteDeviation()), w, rd.X),
        )
        @test isapprox(
        ...
        @test isapprox(
            expected_risk(
                HighOrderMoment(; mu = dot(w, pr.mu), alg = FourthLowerMoment()),
                w,
                rd.X,
            ),
            expected_risk(HighOrderMoment(; alg = FourthLowerMoment()), w, rd.X),
        ...
        @test isapprox(
            expected_risk(
                HighOrderMoment(; mu = dot(w, pr.mu), alg = FourthCentralMoment()),
                w,
                rd.X,
            ),
            expected_risk(HighOrderMoment(; alg = FourthCentralMoment()), w, rd.X),
        ...
        @test isapprox(
            expected_risk(
                HighOrderMoment(;
                    mu = dot(w, pr.mu),
                    alg = HighOrderDeviation(; alg = ThirdLowerMoment()),
                ),
                w,
                rd.X,
            ),
        ...
        @test isapprox(
            expected_risk(AverageDrawdown(), w, rd.X),
            expected_risk(AverageDrawdown(; w = wt), w, rd.X),
        )
        @test isapprox(
            expected_risk(RelativeAverageDrawdown(), w, rd.X),
            0.05118499953858111,
        )
        @test isapprox(
            expected_risk(RelativeAverageDrawdown(), w, rd.X),
        ...
        @test isapprox(
            expected_risk(RelativeMaximumDrawdown(), w, rd.X),
            0.14712227931904298,
        )
        @test isapprox(
            expected_risk(BrownianDistanceVariance(), w, rd.X),
