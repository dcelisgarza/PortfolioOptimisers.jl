@safetestset "Moments" begin
    ...
    @testset "Expected ReturnsResult" begin
        mes = [
            ...
            ShrunkExpectedReturns(;
                alg = BodnarOkhrinParolya(; target = MeanSquareError()),
            ),
            ShrunkExpectedReturns(;
    ...
    @testset "Covariance Estimators" begin
        ces = [
            ...
            MutualInfoCovariance(; bins = Scott()),
            MutualInfoCovariance(; bins = 5),
            MutualInfoCovariance(;
                ve = SimpleVariance(;
                    me = SimpleExpectedReturns(; w = ew),
                    corrected = false,
    ...
    @testset "Regression" begin
        res = [
            StepwiseRegression(; alg = Forward()),
    ...
    @testset "Canonical Distance" begin
        ...
        df = CSV.read(
            joinpath(@__DIR__, "./assets/CanonicalDistanceDistance.csv.gz"),
            DataFrame,
        )
        ...
        for (i, ce) in pairs(ces)
            ...
            elseif isa(ce, LTDCovariance)
                distance(GeneralDistanceDistance(; alg = LogDistance()), cov(ce, rd.X), rd.X)
            else
                distance(deg, cov(ce, rd.X), rd.X)
            end
            success = isapprox(vec(d1), df[!, i])
