...
val = X * w .- dot(w, pr1.mu)
val = val[val .<= zero(eltype(val))]
@test isapprox(expected_risk(r[5], w, X), dot(val, val) / (size(X, 1) - r[5].alg.ddof))
...
@test isapprox(expected_risk(rv[4], wv, prv.X), -sum(val .^ 3) / size(prv.X, 1) / s^3)
...
@test isapprox(expected_risk(rv[5], wv, prv.X), sum(val .^ 4) / size(prv.X, 1) / s^4)
...
val = x .- dot(wv, prv.mu)
s = std(rv[6].alg.ve, x)
@test isapprox(expected_risk(rv[6], wv, prv.X), sum(val .^ 4) / size(prv.X, 1) / s^4)
#     end
# end
