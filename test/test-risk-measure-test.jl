...
using PortfolioOptimisers, Test, Random, StableRNGs, CSV, DataFrames, StatsBase, Clarabel,
      LinearAlgebra
...
rs = [Variance(; settings = settings, formulation = formulation, sigma = sigma, rc = rc),
      Variance(;)]
...
rs = [LowOrderMoment(; settings = settings, w = ew, mu = mu), LowOrderMoment(; mu = 0),
      LowOrderMoment(; mu = zerovec), LowOrderMoment(; alg = SemiDeviation(; ddof = 2)),
...
val = val[val .<= zero(eltype(val))]
@test expected_risk(rs[4], w, X) == sqrt(dot(val, val) / (size(X, 1) - rs[4].alg.ddof))
...
rs = [HighOrderMoment(; settings = settings, w = ew, mu = mu),
      ...
      HighOrderMoment(;
                      alg = HighOrderDeviation(; alg = FourthLowerMoment(),
                                               ve = SimpleVariance(; me = nothing, w = ew))),
