@safetestset "NestedClustering" begin
    ...
    @testset "Mix optimisers" begin
        ...
        opts = [
            ...
            NestedClustering(;
                cle = clr,
                opti = NearOptimalCentering(; opt = jopti),
                opto = NearOptimalCentering(; opt = jopto),
            ),
            NestedClustering(;
                cle = clr,
                opti = RiskBudgetting(; opt = jopti),
                opto = RiskBudgetting(; opt = jopto),
            ),
            NestedClustering(;
                cle = clr,
                opti = RelaxedRiskBudgetting(; opt = jopti),
            ...
            NestedClustering(;
                cle = clr,
                opti = InverseVolatility(; pe = pr),
                opto = InverseVolatility(),
            ),
            NestedClustering(; cle = clr, opti = EqualWeighted(), opto = EqualWeighted()),
            NestedClustering(;
                cle = clr,
                ...
                opti = Stacking(;
                    ...
                    opto = Stacking(;
                        opti = [
                            ...
                            EqualWeighted(),
                            NestedClustering(;
                                opti = NearOptimalCentering(; opt = jopto),
                                opto = NearOptimalCentering(; opt = jopto),
                            ),
                        ],
                        opto = HierarchicalRiskParity(; opt = hopto),
            ...
            NestedClustering(;
                cle = clr,
                opti = FactorRiskContribution(; opt = jopti),
    ...
    @testset "Fees" begin
        fees = FeesEstimator(;
            tn = TurnoverEstimator(; w = w0, val = Dict("PG" => 0.5 / 252)),
            l = Dict("MRK" => 0.3 / 252),
            s = "BAC" => 0.2 / 252,
            fl = ["XOM" => 0.5 / 252],
            fs = "PFE" => 0.3 / 252,
        ...
        res = optimise!(
            NestedClustering(;
                cle = clr,
                opti = MeanRisk(; r = ConditionalDrawdownatRisk(), opt = opti),
                opto = MeanRisk(; opt = opto),
            ),
            rd,
        )
