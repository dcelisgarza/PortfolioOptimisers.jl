    ...
    @testset "MIP Risk Budgeting" begin
        ...
        @test isapprox(res.w,
                       [0.033127220559750765, 0.022769806963934144, 0.03980006959829777,
                        ...
                        0.06560682427264843, 0.05108403002238485], rtol = 1e-6)
        ...
        v1, m1 = findmin(rkc)
        v2, m2 = findmax(rkc)
        success = isapprox(v2 / v1, 20; rtol = 5e-5)
        if !success
            find_tol(v2 / v1, 20)
        end
        @test success
        ...
        @test isapprox(res.w,
                       [0.003291670890764884, 0.0048310396898406495, 0.012062367227361308,
                        ...
                        0.10412048295611062, 0.08726672366225435], rtol = 1e-6)

        rd2 = PortfolioOptimisers.returns_result_getindex(rd, 1:2:20)
        ...
        v1, m1 = findmin(rkc)
        v2, m2 = findmax(rkc)
        success = isapprox(v2 / v1, 10; rtol = 1e-4)
        if !success
            find_tol(v2 / v1, 10)
        end
        @test success
