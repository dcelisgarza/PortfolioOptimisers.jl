@safetestset "Clustering optimisation" begin
    ...
    @testset "HierarchicalRiskParity" begin
        df = CSV.read(
            joinpath(@__DIR__, "./assets/HierarchicalRiskParity1.csv.gz"),
            DataFrame,
        )
        for (i, r) in pairs(rs)
    ...
    @testset "HierarchicalRiskParity vector rm" begin
        sces = [
            SumScalariser(),
            MaxScalariser(),
            LogSumExpScalariser(; gamma = 1.2e2),
            LogSumExpScalariser(; gamma = 1e6),
        ]
        df = CSV.read(
            joinpath(@__DIR__, "./assets/HierarchicalRiskParity2.csv.gz"),
            DataFrame,
        )
    ...
    @testset "HierarchicalEqualRiskContribution" begin
        w1 = [
            0.00908374873880388,
            0.0030813159109630405,
            0.010918199959460204,
            0.005595309855503248,
            0.022247736637590203,
            0.009462220791969355,
            0.01173118247048863,
            0.15335141162301058,
            0.012897796620250668,
            0.1202420807244618,
            0.06262619007540955,
            0.11709370371475523,
            0.009275523953337955,
            0.12297190754199298,
            0.06382989742924242,
            0.09641530015038421,
            0.006088453496011643,
            0.0783381784854751,
            0.06517803092186192,
            0.01957181089902754,
        ]
        ...
        res = optimise!(
            HierarchicalEqualRiskContribution(; threads = FLoops.SequentialEx(), opt = opt),
        )
        @test isa(res.retcode, OptimisationSuccess)
        @test isapprox(res.w, w1)
        res = optimise!(
    ...
    @testset "HierarchicalEqualRiskContribution scalarisers" begin
        sces = [
            SumScalariser(),
            MaxScalariser(),
            LogSumExpScalariser(; gamma = 1e-3),
            LogSumExpScalariser(; gamma = 1e2),
        ]
        df = CSV.read(
            joinpath(@__DIR__, "./assets/HierarchicalEqualRiskContribution2.csv.gz"),
            DataFrame,
        )
        for (i, sce) in pairs(sces)
            res = optimise!(
                HierarchicalEqualRiskContribution(;
                    ri = [
                        ConditionalValueatRisk(),
                        Variance(; settings = RiskMeasureSettings(; scale = 1e1)),
                    ],
                    opt = opt,
                    scei = sce,
                ),
    ...
    @testset "Weight bounds" begin
        ...
        eqn = WeightBoundsEstimator(;
            lb = ["JNJ" => 0.03, "group1" => 0.02],
        ...
        @test all(
            abs.(
                res.w[[
                    findfirst(x -> x == i, sets.dict[sets.key]) for i in sets.dict["group1"]
                ]] .- 0.02,
            ) .<= 1e-10,
        )
        ...
        opt = HierarchicalOptimiser(;
            ...
            sets = sets,
            wb = eqn,
            cwf = JuMPWeightFiniliser(; alg = RelativeErrorWeightFiniliser(), slv = slv),
        )
        res = optimise!(HierarchicalEqualRiskContribution(; opt = opt))
        @test isa(res.retcode, OptimisationSuccess)
        @test isapprox(res.w[findfirst(x -> x == "JNJ", sets.dict[sets.key])], 0.03)
        ...
        opt = HierarchicalOptimiser(;
            ...
            sets = sets,
            wb = eqn,
            cwf = JuMPWeightFiniliser(; alg = AbsoluteErrorWeightFiniliser(), slv = slv),
        )
        res = optimise!(HierarchicalEqualRiskContribution(; opt = opt))
        @test isa(res.retcode, OptimisationSuccess)
        @test isapprox(res.w[findfirst(x -> x == "JNJ", sets.dict[sets.key])], 0.03)
