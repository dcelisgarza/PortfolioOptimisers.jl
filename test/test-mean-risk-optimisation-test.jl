...
ucs1 = sigma_ucs(NormalUncertaintySetEstimator(; pe = EmpiricalPriorEstimator(), rng = rng,
                                               alg = BoxUncertaintySetAlgorithm(),
                                               seed = 987654321), X)
ucs2 = sigma_ucs(NormalUncertaintySetEstimator(; pe = EmpiricalPriorEstimator(), rng = rng,
...
risks = [Variance(; sigma = sigma),#
         ...
         OrderedWeightsArray(; formulation = ExactOrderedWeightsArray(), w = owa_tg(200)),#
...
objs = [MinimumRisk(), MaximumUtility(), MaximumUtility(; l = 1), MaximumRatio(; ohf = 1),
        MaximumRatio(;), MaximumRatio(; rf = rf, ohf = 1), MaximumRatio(; rf = rf),
        MaximumReturn()]
