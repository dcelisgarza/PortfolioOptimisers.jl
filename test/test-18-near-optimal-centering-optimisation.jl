...
@testset "Pareto Surface" begin
    opt = JuMPOptimiser(; pe = pr, slv = slv)
    df = CSV.read(joinpath(@__DIR__, "./assets/NearOptimalCenteringParetoSurface1.csv.gz"),
                  DataFrame)
    r1 = StandardDeviation(;
                           settings = RiskMeasureSettings(; scale = 2e2,
                                                          ub = Frontier(; N = 3)))
    r2 = ConditionalValueatRisk(; settings = RiskMeasureSettings(; ub = Frontier(; N = 5)))
    res1 = optimise!(NearOptimalCentering(; r = [r1, r2], obj = MaximumReturn(), opt = opt))
    success = isapprox(Matrix(df), hcat(res1.w...); rtol = 5e-5)
    if !success
        find_tol(Matrix(df), hcat(res1.w...))
    end
    ...
    df = CSV.read(joinpath(@__DIR__, "./assets/NearOptimalCenteringParetoSurface2.csv.gz"),
                  DataFrame)
    opt = JuMPOptimiser(; pe = pr,
                        ret = ArithmeticReturn(;
    ...
    df = CSV.read(joinpath(@__DIR__, "./assets/NearOptimalCenteringParetoSurface3.csv.gz"),
                  DataFrame)
    ...
    r1 = StandardDeviation(;
                           settings = RiskMeasureSettings(; scale = 2e2,
                                                          ub = Frontier(; N = 3)))
    r2 = ConditionalValueatRisk(; settings = RiskMeasureSettings(; ub = Frontier(; N = 5)))
    res3 = optimise!(NearOptimalCentering(; r = [r1, r2], obj = MaximumReturn(), opt = opt,
                                          alg = ConstrainedNearOptimalCentering()))
