...
function each_cap(f, entries)
    ...
    visit(n::Group) = (n.head isa Cap && f(n.head); foreach(visit, n.children))
...
@testset "Capability catalogue" begin
    ...
    @testset "every exported function is accounted for" begin
        ...
        unaccounted = sort([n
                            for n in fns if !(n in catalogued) && !haskey(NOT_A_FEATURE, n)])
    ...
    @testset "every catalogued name resolves" begin
        ...
        each_cap(CATALOGUE) do c
            for n in c.names
                n isa Symbol && !isdefined(PO, n) && push!(unresolved, n)
            end
        end
    ...
    @testset "verbatim labels agree with their names" begin
        ...
        each_cap(CATALOGUE) do c
            (isnothing(c.label) || !occursin("](@ref)", c.label)) && return nothing
            ...
            isempty(extra) || push!(mismatched, (Symbol(first(c.names)), extra))
            return nothing
        end
