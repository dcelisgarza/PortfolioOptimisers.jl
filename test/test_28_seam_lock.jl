@testset "Seam-lock: prefix-managed keys never read bare" begin
    ...
    # consumes an indicator must take the typed bundle and read it through `held`/`held_bin`/
    # `lb_gate`/…, never `model[:ib]`. Reading by key is exactly the regression that made
    # `set_iplg_constraints!` throw `KeyError(:ib)` on a short + threshold/fee/xbgt model (only
    # the long-only builder registers `:ib`; the long-short builder registers `:ilb`/`:isb`).
    # These keys are also `mip_key`-namespaced under a sub-group space, so a bare read grabs the
    # wrong (or absent) bit. Since ADR 0034 the bundle itself lives in Model State
    # (`set_mip_indicators!`/`mip_indicators`), so even the *late* integer-phylogeny emitter
    # reaches it through the typed accessors rather than a hand-threaded return value — this
    # regex is now a backstop against a fresh bare read, not the only thing holding the seam.
